...
var elesmallcor = {
	 ...
	 */
	init : function(){
		grid.init("eleSmallCor");
		loading.init();
		try{
			elesmallcor.createEleSmallCorGrid();
			elesmallcor.requestEleSmallCorData();
		}
		catch(err){
			loading.close();
		}
	 ...
	 * 构建区域管理列表列集合
	 */
	createEleSmallCorColumns : function(){
		grid.resetColumn();
	 ...
	 * 创建区域管理列表
	 */
	createEleSmallCorGrid : function(){
		var columns = elesmallcor.createEleSmallCorColumns();
	 ...
	 * 发送数据请求
	 */
	requestEleSmallCorData : function(){
		common.init("../electronsmallcor/elesmallcorlist","POST",elesmallcor.bindEleSmallCorGrid);
		common.do_submit();
	},
	...
	toStatusPage : function(tr){
		...
		var task_Id='';   //修空需要传值
		// 跳转到区域管理编辑页面
		common.toPage("../electronsmallcor/initstatus?process_Inst_Id="+process_Inst_Id+"&task_Id"+task_Id);
	},
	 ...
	 * 这是一个回调函数，不用手动调用
	 */
	bindEleSmallCorGrid : function(result){
