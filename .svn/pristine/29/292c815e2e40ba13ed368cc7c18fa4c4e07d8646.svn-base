...
var templete = {
	...
	init : function(){
		...
		loading.init();
		try{
			templete.createTempleteGrid();
			templete.requestTempleteData();
		}
		catch(err){
			loading.close();
		}
	 ...
	 * 构建区域管理列表列集合
	 */
	createTempleteColumns : function(){
		grid.resetColumn();
	 ...
	 * 创建区域管理列表
	 */
	createTempleteGrid : function(){
		var columns = templete.createTempleteColumns();
	 ...
	 * 发送数据请求
	 */
	requestTempleteData : function(){
		common.init("../templete/templetelist","POST",templete.bindTempleteGrid);
		common.do_submit();
	},
	...
	toStatusPage : function(tr){
		...
		var id = rowData.id;
		// 跳转到区域管理编辑页面
		common.toPage("../templete/initstatus?id="+id);
	},
	...
	toDetailsPage : function(tr){
		...
		var id = rowData.id;
		// 跳转到区域管理编辑页面
		common.toPage("../templete/initdetails?id="+id);
	},
	 ...
	 * 这是一个回调函数，不用手动调用
	 */
	bindTempleteGrid : function(result){
