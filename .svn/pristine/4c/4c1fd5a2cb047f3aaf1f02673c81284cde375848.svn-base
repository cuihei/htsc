package com.ht.action.complication.smallcorrection.electron;
...
public class EleSmallCorAction extends BaseAction{
	
	/**
	 * 改正通告service
	 * */
	@Resource
	private EleSmallCorService eleSmallCorService;
	 ...
	 * 获取改正通告list
	 */
	public void findEleSmallCorList(){
		try {
			List<SmallCor> list = eleSmallCorService.findEleSmallCorList();
	 ...
	 * 获取源数据一条流转状态
	 */
	public void findEleSmallCorStatus(){
		try {
			//流程实例ID
			String process_Inst_Id = getParam("process_Inst_Id");
			//任务ID
