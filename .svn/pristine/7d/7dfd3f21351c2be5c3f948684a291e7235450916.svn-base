...
public class DynamicTablePage {
	...
	public String getListPage() {
		...
		util.createHeaderBar(root, "港口航道图月度在编动态");
		...
		InputGroup ig = (InputGroup) util.createRangeDatePickerGroup("计划汇交时间", "date1", "请输入计划汇交开始日期", "date2", "请输入计划汇交结束日期");
		...
		/** 创建Grid行  开始*/
		// 创建Grid
		Base rowGrid = util.createGrid(root,"dynamic_table");
		/** 创建Grid行  结束*/
		//返回节点字符串
		return root.getNode();
	}
