package com.ht.action.complication.project.electron;
...
public class ProElectronAction extends BaseAction{
	
	/**
	 * 海图编绘service
	 * */
	@Resource
	private ProElectronService proElectronService;
	 ...
	 * 获取电子海图list
	 */
	public void findProEleList(){
		try {
			List<Project> list = proElectronService.findProEleList();
	 ...
	 * 获取源数据一条流转状态
	 */
	public void findProElectronStatus(){
		try {
			//流程实例ID
			String process_Inst_Id = getParam("process_Inst_Id");
			//任务ID
