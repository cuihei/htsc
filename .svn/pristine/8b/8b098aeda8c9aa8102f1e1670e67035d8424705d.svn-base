...
public class CorrectionNoticeServiceImpl implements CorrectionNoticeService {
	...
	//@Override
	public List<CorrectionNotice> queryNoticeByCondition1(String ChartNo, String key,
	...
	public List<CorrectionNotice> queryNoticeByCondition(String ChartNo, String key,
		 ...
		 * 拼接查询条件
		*/
		String strWhere = "iss.State=1  and TO_CHAR(iss.year) = y.name and i.notice_periods_id=iss.ID  and i.ActID=a.ID and i.NoticeTypeID=n.ID";
		...
		if(null!=key&&""!=key){
			...
			//拼入关键字
			for (String k : keys) {
				strWhere = strWhere + " and (i.DocumentContent LIKE '%" + k.trim() + "%' or i.Title  LIKE '%" + k.trim() + "%')";
			}
		}
		//拼入图号
		if(null!=ChartNo&&""!=ChartNo){
			strWhere = strWhere + " and i.figure_no LIKE '%" + ChartNo.trim()+ "%'";	
		}
		/*//拼入开始时间
		if(null!=startTime&&""!=startTime){
			 strWhere = strWhere + " and i.CreateTime >= to_date('" +startTime.trim()+ "','yyyy-MM-dd')";
		...
		String tableName = " (select y.ID yearid,y.Name YearName,i.ID,i.notice_periods_id IssueID,i.item_no ItemNo,i.create_date CreateTime,i.Title,i.figure_no ChartNo,iss.num IssueNo,iss.num_total TotalIssueNo,iss.release_date ReleaseTime,a.Name as Act,n.Name as NoticeType,y.name || lpad(substr(i.Item_No,instr(i.Item_No,'-') +1),5,'0') as yearitemno from  Year y,da_notice_basic_info i,DA_NOTICE_PERIODS iss,Act a,NoticeType n where 1=1 and " + strWhere + ") M ";
