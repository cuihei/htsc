package com.ht.front.pages.background.monitor.accesslog;
...
public class Accesslog {
	...
	public String getListNode() {
		...
		util.createHeaderBar(root, "系统访问日志管理");
		...
		/** 创建Grid行  开始*/
		// 创建Grid
		Base rowGrid = util.createGrid(root,"syslog");
		/** 创建Grid行  结束*/
		return root.getNode();
	}
	...
	public String getAddNode() {
		...
		param.add(tb);
		tb = InputGroup.getInGroup("访问者id", "handleId", null, "请输入访问者id");
		param.add(tb);
		tb = InputGroup.getInGroup("访问者ip", "handleIp", null, "请输入访问者ip");
		param.add(tb);
		tb = InputGroup.getInGroup("操作行为", "handleBehavior", null, "请输入操作行为");
		param.add(tb);
		tb = InputGroup.getInGroup("操作结果", "handleResult", null, "请输入操作结果");
		param.add(tb);
		InputGroup ta = InputGroup.getDatePicker("handleTime");
		param.add(ta);
		Base editPage = edit.createEditPage(param);
		FrontUtil util = FrontUtil.getInstance();
		util.createHeaderBar(editPage, "访问日志创建");
		//返回节点字符串
