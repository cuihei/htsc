...
public class YearTaskBookPage {
	 ...
	 * @throws Exception 
	 */
	public String getListPage(TaskBookService taskBookService,BaseDataService baseDataService) throws Exception {
		// 获取根节点
		this.root = util.createRoot();
		util.createHeaderBar(this.root, "年度任务书");
		...
		Button btn = Button.getInstance("submit", css, "发布");
		...
		List<TaskBook> booklist = taskBookService.findListCorrectionNoticeByYear(year);
		root.addChildNode(InputHidden.getInstance("count", String.valueOf(booklist.size()+1)));
	 ...
	 * @throws Exception
	 */
	public String getEditPage(String taskBookType,
			TaskBookService taskBookService,
			OrganizationService organizationService, String id,
			String processInstId, String taskId, String processDefId,
			String taskDefId, VProcessDetailDao vProcessDetailDao,
			TaskBookPlanRelationService taskBookPlanRelationService,BaseDataService baseDataService)
			throws Exception {
		TaskBook taskbook = new TaskBook();
		String planId = null;
		if (StringUtils.isNotEmpty(processInstId)) {
			if (StringUtils.isEmpty(id)) {
				VProcessDetail process = vProcessDetailDao
						.getProcessDetailByProcessIdAndId(processInstId,
								taskId, processDefId, taskDefId);
				if (process != null) {
		...
		if (StringUtils.isNotEmpty(id)) {
			...
			List<TaskBookPlanRelation> list = taskBookPlanRelationService
					.findListByBookId(taskbook.getId());
		...
		// 隐藏id
		root.addChildNode(InputHidden.getInstance("taskBookId",
				taskbook.getId()));
		
		Base row = util.createRow(root);
		Base col = util.createColumn(row, "10", "10", "4", null);
		String[] number = { "&#12288一、", "&#12288二、", "&#12288三、", "&#12288四、", "&#12288五、", "&#12288六、", "&#12288七、", "&#12288八、",
				"&#12288九、", "&#12288十、", "十一、", "十二、", "十三、", "十四、", "十五、" ,"十六、"};
		int i = 0;
		List<BaseData>  taskBookTypeList = baseDataService.getBaseDataByTypeId(BaseDataConstants.TASK_BOOK_TYPE);
		Base sg = util.creatDefaultSelectGroup("&#12288&#12288&#12288任务书类型","typeId",taskBookTypeList,"code", "value",taskBookType);
		col.addChildNode(sg);
		// 沪印绘 任务书的数量
				long hyh = 0;
		...
		// 设置任务书编号
				Span taskbookNo = Span.getInstance(new CssClass("page_footer"), hyhNo);
				Prop prop = new Prop();
				prop.setPropKey("id");
				prop.setPropValue("taskbookNo");
				taskbookNo.addProp(prop);
		// 一、选择任务：级联选择
		Div div = Div.getInstance(
				"",
				new CssClass("input-group"),
				Span.getDefault(number[i++] + "&#12288选择任务：").getNode()
						+ Button.getInstance("chooseTask",
								new CssClass("btn btn-primary"), "选择任务")
								.getNode()+taskbookNo.getNode());
		col.addChildNode(div);
		// 创建选择任务页面
		this.createModalDialog();
		util.createGrid(col, "taskShow");
		// 二、任务名称：
		col.addChildNode(InputGroup.getInGroup(number[i++] + "任务书名称",
				"taskbookName", taskbook.getTaskbookName(), "请输入任务书名称"));
		// 三、任务来源：
		String taskFrom = taskbook.getTaskFrom();
		if (null == taskFrom || "" == taskFrom) {
			taskFrom = "部局" + sdf.format(date) + "年计划";
		}
		col.addChildNode(InputGroup.getInGroup(number[i++] + "&#12288任务来源",
				"taskFrom", taskFrom, "请输入任务来源"));
		// (四)：根据类型显示匹配字段
		if ("TASK_BOOK_PROJECT_SPECIAL".equals(taskBookType)) {
			col.addChildNode(InputGroup.getInGroup(number[i++] + "&#12288项目名称",
					"itemName", taskbook.getItemName(), "请输入项目名称"));
		} else if ("TASK_BOOK_SMALL_CORRECTION".equals(taskBookType)) {
			col.addChildNode(InputGroup.getInGroup(number[i++] + "改正通告期号",
					"correctNoticeNo", taskbook.getCorrectNoticeNo(),
					"请输入改正通告期号"));
			col.addChildNode(InputGroup.getInGroup(number[i++] + "&#12288改正期号",
					"correctNo", taskbook.getCorrectNo(), "请输入改正期号"));
		} else if (("TASK_BOOK_CORRECTION_NOTICE").equals(taskBookType)) {
			col.addChildNode(InputGroup.getInGroup(number[i++] + "&#12288本期项号",
					"correctItemNo", taskbook.getCorrectItemNo(), "请输入本期项号"));
			col.addChildNode(InputGroup.getInGroup(number[i++] + "&#12288工程名称",
					"projectName", taskbook.getProjectName(), "请输入工程名称"));
			// 通告期号
			String noticeNo = taskbook.getNoticeNo()==null?"1":taskbook.getNoticeNo();
			col.addChildNode(InputGroup.getInGroup(true, number[i++] + "&#12288通告期号",
					"noticeNo", noticeNo, "请输入通告期号"));
		}
		// 四、执行部门：
		List<Organization> organizationList = organizationService
				.getOrganization();// "executeDeptId", organization, "id",
									// "orgName",false
		...
		for (Organization organization2 : organizationList) {
			if (organization2.getParentId() != null
					&& organization2.getParentId().equals("1019142929270002")) {
				organization.add(organization2);
			}
		}
		if (taskbook.getExecuteDeptId() != ""
				&& taskbook.getExecuteDeptId() != null) {
			div = Div.getInstance("",null,InputGroup.getSelectGroup((number[i++] + "&#12288执行部门"), "executeDeptId",organization, "id", "orgName", "false").getNode());
		} else {
			div = Div.getInstance(
					"",
					new CssClass("input-group"),
					Span.getDefault(number[i++] + "&#12288执行部门").getNode()
							+ Select.getDefaultWithOption("executeDeptId",
									organization, "id", "orgName",
									"10311115414710008").getNode());
		}
		col.addChildNode(div);
		// 五、执行日期：
		// 执行日期默认当前时间
        ...
        InputGroup ig = InputGroup.getDatePicker(number[i++] + "&#12288执行日期","executeTime", nowTime, "请选择执行日期");
		col.addChildNode(ig);
		// 五、结束日期：
		String endTime = "";
		...
		ig = InputGroup.getDatePicker(number[i++] + "&#12288结束日期","endTime", endTime, "请选择结束日期");
		col.addChildNode(ig);
		// 六、技术标准：text
		div = Div.getInstance("", new CssClass("input-group"), "");
		div.addChildNode(Span.getDefault(number[i++] + "&#12288技术标准："));// "technologyStandard","10"
		List<BaseData> technologyStandardList = taskBookService
				.getTechnologyStandard();
		if (null != taskbook && null != taskbook.getTechnologyStandard()) {
			div.addChildNode(TextArea.getInstance("technologyStandard",
					new CssClass("form-control"), "10",
					taskbook.getTechnologyStandard()));
			...
			div.addChildNode(TextArea.getInstance("technologyStandard",
					new CssClass("form-control"), "10", technologyStandard));
		} else {
			div.addChildNode(TextArea.getInstance("technologyStandard",
					new CssClass("form-control"), "10", ""));
		}
		...
		// 七、技术要求：
		div = Div.getInstance("", new CssClass("input-group"), "");
		div.addChildNode(Span.getDefault(number[i++] + "&#12288技术要求："));
		List<BaseData> technologyDemandList = taskBookService
				.getTechnologyDemand();
		if (null != taskbook && null != taskbook.getTechnologyDemand()) {
			div.addChildNode(TextArea.getInstance("technologyDemand",
					new CssClass("form-control"), "10",
					taskbook.getTechnologyDemand()));
			...
			div.addChildNode(TextArea.getInstance("technologyDemand",
					new CssClass("form-control"), "10", technologyDemand));
		} else {
			div.addChildNode(TextArea.getInstance("technologyDemand",
					new CssClass("form-control"), "10", ""));
		}
		...
		// 七、版次：
		/*div = Div.getInstance("", new CssClass("input-group"), "");
		div.addChildNode(Span.getDefault(number[i++] + "版次： "));
		List<BaseData> vsersionList = taskBookService.getVersion();
		if (null != taskbook && null != taskbook.getRevision()) {
			div.addChildNode(TextArea.getInstance("revision",
					new CssClass("form-control"), "1",
					taskbook.getRevision()));
		} else if (vsersionList.size() != 0) {
			String revision = "";
			for (BaseData baseData : vsersionList) {
				if ("" != revision) {
					revision += "\n" + baseData.getValue();
				} else {
					revision += baseData.getValue();
				}
			}
			div.addChildNode(TextArea.getInstance("revision",
					new CssClass("form-control"), "1", revision));
		} else {
			div.addChildNode(TextArea.getInstance("revision",
					new CssClass("form-control"), "1", ""));
		}
		...
		// 八、其他要求：
		div = Div.getInstance("", new CssClass("input-group"), "");
		div.addChildNode(Span.getDefault(number[i++] + "&#12288其他要求："));
		List<BaseData> otherDemandList = taskBookService.getOtherDemand();
		if (null != taskbook && null != taskbook.getOtherDemand()) {
			div.addChildNode(TextArea.getInstance("otherDemand", new CssClass(
					"form-control"), "10", taskbook.getOtherDemand()));
		...
		col.addChildNode(div);
//		// 沪印绘 任务书的数量
//		long hyh = taskBookService.countByNo("沪印绘");
//		// 沪印临 任务书的数量
//		long hyl = taskBookService.countByNo("沪印临");
//		// 拼接任务书序号
//		String hyhNo = "";
//		String hylNo = "";
//		hyhNo = sdf.format(date) + "沪印绘" + formatNo(String.valueOf(hyh)) + "号";
...
//		}
		...
		col.addChildNode(Span.getInstance(new CssClass("page_footer"),
				sdf.format(date)));
		// 保存，返回按钮
		ButtonGroup buttonGroup = ButtonGroup.getInstance(this
				.createButtonList(true, true, false));
	 ...
	 * @throws Exception
	 */
	public String getDetailsPage(TaskBook taskbook,
		...
		util.createGrid(col, "taskShow");
		// 二、任务名称：
		col.addChildNode(InputGroup.getInGroup("二、任务书名称：", null,
				taskbook.getTaskbookName(), ""));
		// 三、任务来源：
		col.addChildNode(InputGroup.getInGroup("三、任务来源：", null,
				taskbook.getTaskFrom(), ""));
		// 四、执行部门：
		Organization organization = organizationService
				.getOrganization(taskbook.getExecuteDeptId());// "executeDeptId",
																// organization,
																// "id",
																// "orgName",false
		...
		// 五、执行日期：
		col.addChildNode(InputGroup.getInGroup("五、执行日期：", null,
				taskbook.getExecuteTime() + "", ""));
		// 六、技术标准：text
		div = Div.getInstance("", new CssClass("input-group"), "");
		...
		div.addChildNode(TextArea.getInstance("technologyDemand", new CssClass(
				"form-control"), "10", taskbook.getTechnologyDemand()));
		col.addChildNode(div);
		// 八、其他要求：
		div = Div.getInstance("", new CssClass("input-group"), "");
		...
		col.addChildNode(div);
		// 设置任务书编号
		Span taskbookNo = Span.getInstance(new CssClass("page_footer"),
				taskbook.getTaskbookNo());
		...
		// 保存，返回按钮
		ButtonGroup buttonGroup = ButtonGroup.getInstance(this
				.createButtonList(true, false, false));
		col.addChildNode(util.createRowSpace());
		col.addChildNode(buttonGroup);
		return root.getNode();
	}
	...
	public String createButtonInScript(CssClass btnCss, CssClass iCss,
		...
		// 为按钮添加属性
		btn.addProp(prop);
		if (btnName.equals("histroy")) {// 历史版本
			prop = new Prop();
	...
	public String createButtonInGrid() {
		// 历史版本按钮
		String buttons = this.createButtonInScript(new CssClass(
				"btn btn-primary"), new CssClass("fa fa-github-alt"),
				"histroy", "hisTemplate");
		// 上传附件按钮

		buttons += this.createButtonInScript(new CssClass(
				"btn btn-primary btn-setting"), new CssClass("fa fa-upload"),
				"upload", "uploadTemplate");
		
		// 下载附件按钮
		buttons += this.createButtonInScript(new CssClass(
				"btn btn-primary btn-setting"), new CssClass("fa fa-download"),
				"download", "downloadTemplate");
		
		// 编辑按钮
		buttons += this.createButtonInScript(new CssClass("btn btn-success"),
				new CssClass("fa fa-edit"), "edit", "editTemplate");
		// 删除按钮
	...
	public List<Button> createButtonList(boolean isBack, boolean isSubmit,
		...
		// 如果条件成立，创建新增按钮。
		if (isAdd) {
			btnList.add(Button.getInstance("addTaskBook", new CssClass(
					"btn btn-success"), "新增"));
		}
		// 如果条件成立，创建提交按钮。
		if (isSubmit) {
			btnList.add(Button.getInstance("submit", new CssClass(
					"btn btn-success"), "提交"));
		}
		// 如果条件成立，创建返回按钮。
		if (isBack) {
			btnList.add(Button.getInstance("backPage", new CssClass(
					"btn btn-default"), "返回"));
		}
		return btnList;
	}
	...
	public void createModalDialog() {
		...
		/*
		 * Prop closeProp = new Prop(); closeProp.setPropKey("data-dismiss");
		 * closeProp.setPropValue("modal"); Prop closeProps = new Prop();
		 * closeProps.setPropKey("aria-hidden");
		 * closeProps.setPropValue("true"); // 绑定属性 closeBtn.addProp(closeProp);
		...
		// 创建返回按钮。
		Button backBtn = Button.getInstance("hiddenModal", new CssClass(
				"btn btn-default"), "返回");
