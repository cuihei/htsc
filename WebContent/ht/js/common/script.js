var App = function () {
	...
	jQuery('.sidebar-menu .has-sub > a').click(function () {
            ...
            if (sub.is(":visible")) {
				...
				sub.slideUp(slideSpeed,setGridHeight);
            } else {
                jQuery('.arrow', jQuery(this)).addClass("open");
                jQuery(this).parent().addClass("open");
                sub.slideDown(slideSpeed,setGridHeight);
            }
        });
	...
	jQuery('.sidebar-menu .has-sub .sub .has-sub-sub > a').click(function () {
            ...
            if (sub.is(":visible")) {
                jQuery('.arrow', jQuery(this)).removeClass("open");
                jQuery(this).parent().removeClass("open");
                sub.slideUp(300,setGridHeight);
            }
            // 展开
            else {
                jQuery('.arrow', jQuery(this)).addClass("open");
                jQuery(this).parent().addClass("open");
                sub.slideDown(300,setGridHeight);
            }
        });
	}
...
//		}
//		else{
//			$("#page").css("height", $("#index-body").height()-$("#header").height());
//		}
//		var fullHeight = $("#page").height();
//		var otherHeight = 0;
//		var fram = $(document.getElementById("page").contentWindow.document);
//		var divs = fram.find(".container-fluid").children("div");
//		for (var int = 0; int < divs.length; int++) {
//			var div = divs[int];
//			var css = $(div).attr("class");
//			if (css != null) {
//				if (css.indexOf("bg-grid-container")<0 && css.indexOf("modal fade")<0 && css != "top35") {
...
//		// 设置grid高度
//        var grid = $(document.getElementById("page").contentWindow.document).find(".bg-grid-container");
//        if (grid != null && grid.length>0) {
//    		var heightHis = grid.height();
//        	grid.css("height",height);
//        	var contentHeightHis = grid.find(".k-grid-content").height();
//        	grid.find(".k-grid-content").css("height",contentHeightHis+(height-heightHis));
//		}
	}
	...
	var collapseSidebar = function () {
		...
		jQuery('#page').addClass("margin-left-50");
	...
	var responsiveSidebar = function () {
		...
		if ( width < 768 ) {
			...
			jQuery('#page').addClass("margin-left-0");
		}
		else {
			is_mobile = false;
			//Show the sidebar completely
			jQuery('#page').removeClass("margin-left-0");
			var menu = $('.sidebar');
...
//		var height =  document.body.clientHeight;
		...
		jQuery('#sidebar-top').css("height",height);
        if ($.cookie('mini_sidebar') === '1') {
			/* For Navbar */
			jQuery('.navbar-brand').addClass("mini-menu");
			/* For sidebar */
			jQuery('#sidebar').addClass("mini-menu");
			jQuery('#sidebar-top').addClass("mini-menu");
			jQuery('#sidebar').removeClass("sidebar-overflow-auto");
			jQuery('#sidebar-top').removeClass("sidebar-overflow-hidden");
			jQuery('#page').addClass("margin-left-50");
			collapsed = true;
			$(".sidebar-menu").css("height",height-50);
			jQuery('#page').addClass("margin-left-50");
        }
        else{
        	jQuery('#sidebar').addClass("sidebar-overflow-auto");
        	$("#page").css("width",width-235);
        	$(".sidebar-menu").css("height",1200);
        }
			...
			if(is_mobile && !(is_mini_menu)){
				...
				if(collapsed){
					...
					if(is_fixed_header) {
						...
						jQuery('#page').addClass("margin-top-100");
					}
					collapsed = false;
					...
					if(is_fixed_header) {
						...
						jQuery('#page').removeClass("margin-top-100");
					}
					collapsed = true;
					$.cookie('mini_sidebar', '1');
					handleMobileSidebar();
				...
				if(collapsed){
					...
					jQuery('#sidebar-top').removeClass("mini-menu");
					jQuery('#sidebar').addClass("sidebar-overflow-auto");
					jQuery('#sidebar-top').addClass("sidebar-overflow-hidden");
					jQuery('#page').removeClass("margin-left-50");
	...
	var handleNavbarFixedTop = function () {
		if(is_mobile && is_fixed_header) {
			//Manage margin top
			$('#page').addClass('margin-top-100');
		}
		if(!(is_mobile) && is_fixed_header){
			//Manage margin top
			$('#page').removeClass('margin-top-100').addClass('margin-top-50');
		}
	} 
	/*-----------------------------------------------------------------------------------*/
	...
	var handleThemeSkins = function () {
        ...
        var setSkin = function (color) {
            $('#skin-switcher').attr("href", "../ht/css/themes/" + color + ".css");
	...
	return {
        ...
        init: function () {
			...
			handleSidebarCollapse(); //Function to hide or show sidebar
			  //Function to hide sidebar and main content height
