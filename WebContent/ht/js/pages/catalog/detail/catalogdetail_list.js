...
var catalog_detail = {
	...
	init : function(){
		...
		function gotoDate(){
			var catalog = {};
			var type = {};
			type.id = $("#categoryId").val();
			var status = $("#status").val();
			if(status != null && status != "" && status.indexOf("请选择")<= -1){
			...
			var year = $("#year").val();
			loading.init();
			common.init("../detail/list?year="+year,"POST",catalog_detail.bindDetailGrid);
	...
	createDetailGrid : function(){
		var type = $("#type").val();
		var columns = catalog_detail.createDetailColumns(type);
	 ...
	 * 构建目录列表列集合
	 */
	createDetailColumns : function(type){
		if(type=="1"){
			grid.resetColumn();
			grid.addColumn("150px","areaName","海区");
			...
			grid.addColumn("100px","nature","性质");
			grid.addColumn("150px","measurementPeriod","基测");
			grid.addColumn("150px","testIng","检测");
			grid.addColumn("150px","publicationYear","出版周期");
			grid.addColumn("150px","datumLatitude","基准纬度");
			grid.addColumn("100px","mapProportion","图积");
			grid.addColumn("150px","adjustmentProperty","调整性质");
			grid.addColumn("100px","publicationDate","出版日期","#= publicationDate ? kendo.toString(new Date(publicationDate), 'yyyy-MM') : '' #" );
			//grid.addColumn("100px","printNum","印次");
			grid.addColumn("150px","remarks","备注",null,null,null,false);
			...
			if($("#jurisdiction").val()=="true"){
				...
				return grid.addColumn("100px","handle","区域图片",kendo.template($("#viewTemplate").html()));
			}
		}else{
			grid.resetColumn();
			grid.addColumn("100px","areaName","海区");
			grid.addLockedColumn("100px","mapNo","图号");
			grid.addLockedColumn("100px","mapName","图名");
			grid.addColumn("100px","scale","比例尺 1:");
			grid.addColumn("150px","starLatitude","图幅范围（纬度）");
	...
	removeDetails : function(){
		...
		layer.confirm('确认要删除吗？',function(index){
			...
			for (var k = 0; k < details.length; k++) {
				if(details[k].status == "待审核"){
					layer.msg("所选目录有数据待审核中，不能删除！");
					return;
				}
			}
	...
	toDetailEditPage : function(tr){
		...
		var categoryId = $("#categoryId").val();
		// 跳转到目录编辑页面
		common.toPage("../detail/init_edit?flag=2&id="+id+"&type="+type+"&categoryId="+categoryId);
	},
	...
	bindPageEvent : function(){
		...
		$("#search").on("click",function(){
			...
			if(lastword1 == "N"){
				...
				if(array.length==1){
					...
					star1 = (Number(array[0])+90)+array[1]/60;
				}else{
					star1 = (Number(array[0])+90)+array[1]/60+array[2]/60/60;
				}
				star1 = star1;
			}else if(lastword1 == "S"){
				array = star1.split("-");
				...
				if(array.length==1){
					...
					starLatitude = (Number(array[0])+90)+array[1]/60;
				}else{
					starLatitude = (Number(array[0])+90)+array[1]/60+array[2]/60/60;
				}
				star1 = starLatitude;
			}
			if(array.length==1){
			...
			if(lastword2 == "N"){
				...
				if(array2.length==1){
					...
					end = (Number(array2[0])+90)+array2[1]/60;
				}else{
					end = (Number(array2[0])+90)+array2[1]/60+array2[2]/60/60;
				}
				end = end;
			}else if(lastword2 == "S"){
				array2 = end.split("-");
				...
				if(array2.length==1){
					...
					endLatitude = (Number(array2[0])+90)+array2[1]/60;
				}else{
					endLatitude = (Number(array2[0])+90)+array2[1]/60+array2[2]/60/60;
				}
				end = endLatitude;
			}
			if(array2.length==1){
			...
			if(lastword3 == "E"){
				...
				if(array3.length==1){
					...
					star2 = (Number(array3[0])+180)+array3[1]/60;
				}else{
					star2 = (Number(array3[0])+180)+array3[1]/60+array3[2]/60/60;
				}
				star2 = star2;
			}else if(lastword3 == "W"){
				array3 = star2.split("-");
				...
				if(array3.length==1){
					...
					starLongitude = (Number(array3[0])+180)+array3[1]/60;
				}else{
					starLongitude = (Number(array3[0])+180)+array3[1]/60+array3[2]/60/60;
				}
				star2 = starLongitude;
			}
			if(array3.length==1){
			...
			if(lastword4 == "E"){
				...
				if(array4.length==1){
					...
					end2 = (Number(array4[0])+180)+array4[1]/60;
				}else{
					end2 = (Number(array4[0])+180)+array4[1]/60+array4[2]/60/60;
				}
				end2 = end2;
			}else if(lastword4 == "W"){
				array4 = end2.split("-");
				...
				if(array4.length==1){
					...
					endLongitude = (Number(array4[0])+180)+array4[1]/60;
				}else{
					endLongitude = (Number(array4[0])+180)+array4[1]/60+array4[2]/60/60;
				}
				end2 = endLongitude;
			}
			if(array4.length==1){
		...
		$("#audit").on("click",function(){
			...
			for (var int = 0; int < res.length; int++) {
				if(res[int].status != '创建'&&res[int].status != '退回'){
					layer.msg("只能提交状态为创建或者退回的数据,请重新选择！");
					return;
				}
			}
