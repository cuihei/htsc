			...
			if (type == "img") {
				colmuns = grid.addColumn("150px",key,name,getImgTemplate(formId,key));
				  /* colmuns = grid.addColumn("120px", "formBb", "历史版本");
				   colmuns =grid.addColumn("150px","creationDate","创建日期 ","#= creationDate ? kendo.toString(new Date(creationDate), 'yyyy-MM-dd') : '' #");
		*/	}
			else if(type == "file"){
		...
		for (var int = 0; int < rowFlags.length; int++) {
			...
			for (var int2 = 0; int2 < formValueData.length; int2++) {
				...
				if (flag == rowFlags[int]) {
					...
					data["formBb"]=prop.formBb;
			        data["creationDate"]=prop.creationDate;
					data[key] = value;
				}
			}
			datas.push(data);
