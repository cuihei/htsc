...
var proelectron = {
	 ...
	 */
	init : function(){
		grid.init("proElectron");
		loading.init();
		try{
			proelectron.createProElectronGrid();
			proelectron.requestProElectronData();
		}
		catch(err){
			loading.close();
		}
	 ...
	 * 构建区域管理列表列集合
	 */
	createProElectronColumns : function(){
		grid.resetColumn();
	 ...
	 * 创建区域管理列表
	 */
	createProElectronGrid : function(){
		var columns = proelectron.createProElectronColumns();
	 ...
	 * 发送数据请求
	 */
	requestProElectronData : function(){
		common.init("../proelectron/proelectronlist","POST",proelectron.bindProElectronGrid);
		common.do_submit();
	},
	 ...
	 * 这是一个回调函数，不用手动调用
	 */
	bindProElectronGrid : function(result){
