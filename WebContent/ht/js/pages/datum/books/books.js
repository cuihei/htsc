	...
	removeBooks : function(){
		...
		layer.confirm('确认要删除吗？',function(index){
			...
			$.each(rowDatas,function(i,item){
				...
				var entry = $("#books").data("kendoGrid").dataItem(item).entry;
				var creator = $("#books").data("kendoGrid").dataItem(item).creator;
				///如果是金忠伟  只能编辑自己创建的。
		        if(u=='029643'&&(entry!="金忠伟"||creator!='029643')){
		        	jzw = true;
			     }
	...
	bindPageEvent : function(){
		...
		//1、表示不是资料管理员   0当前登陆为资料管理员
            if(flag==1){
							...
							common.toPage("../books/batch_borrowing_init?ids="+ids+"&userNo="+userNo);
							});
						...
						$("#commit").on("click",function(){
						   	var uNo=$("#user").val();
							common.toPage("../books/batch_borrowing_init?ids="+ids+"&userNo="+uNo);
						});
		            	}
