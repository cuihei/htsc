...
function editPage(obj) {
	...
	if(u=='029643'&&creator=="029643"){
		fileddata.editFiledDataPage(tr);
	}else if(u=='029643'&&creator!="029643"){
		layer.msg('对不起！权限不足。');
		return false;
     }else if(u!='029643'){
    	fileddata.editFiledDataPage(tr);
	...
	removeFiledData : function(){
		...
		layer.confirm('确认要删除吗？',function(index){
			...
			$.each(rowDatas,function(i,item){
				...
				//var entry = $("#fileddata").data("kendoGrid").dataItem(item).entry;
				var creator = $("#fileddata").data("kendoGrid").dataItem(item).creator;
				///如果是金忠伟  只能编辑自己创建的。
		        if(u=='029643'&&creator!='029643'){
		        	jzw = true;
			     }
