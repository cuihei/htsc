$(function(){
	...
	//$("#mapNo").attr("onkeyup","value=(if((event.keyCode<48 || event.keyCode>57) && event.keyCode!=46 || //./d/d$/.test(value))event.returnValue=false)");
	...
	//图号失去焦点，进行正则判断，是否包含字母 2018.4.3
	  $("#mapNo").blur(function(){
			    	 var val =  $("#mapNo").val();
			         var reg = /([\u4E00-\u9FA5]|[A-Za-z])+/;//只要包含中文或者字母就提示
			        if(reg.test(val)){
			        	//layer.msg("禁止输入中文或字母！且图号重复，将不能保存！");
			        	layer.alert('1、图号禁止输入中文或字母，请返回重新填写。</br>2、若图号已存在，将不能保存成功。')
			        	$("#mapNo").val("")
			//   	$("#mapNo").focus()；
			        return false;
			        }
