...
var tbedit={
		...
		bindPageEvent : function(){
			...
			$("#submit").on("click",function(){
				...
				console.log(executeDeptId)
				var executeTime=$("#executeTime").val();
				var endTime=$("#endTime").val();
				var sjhjTime=$("#sjhjTime").val();
...
var taskDiv={
		...
		createtaskDivColumns : function(){
			...
			grid.addColumn("85px","deliverTime","资料汇交时间","#=deliverTime?kendo.toString(new Date(deliverTime),'yyyy-MM-dd'):'' #");
		...
		bindPageEvent : function(){
			...
			$("#taskSelect").unbind("change").change(function(){
				...
				try{
					...
					$.ajax({
						type : "GET",
						url :"../taskbook/taskselect?taskselect="+$("#taskSelect").val(),
						async : false,
					    dataType:"text",
			             success:function(result){
			                result = eval("("+result+")");
							 if (result.code == 1) {
							//2019沪印绘1607号
							var no=parseInt(result.value)+1;
							var date = new Date();
					    	var bhn=date.getFullYear();	
					    	
					    	if(no<10){no="00"+no;};
					    	if(no<100&&no>10){no="0"+no;};
					    						    	
					    	if($("#taskSelect").val()=="year"){
					    		$("#taskbookNo").val(bhn+"沪印绘"+no+"号")
					    	}else{
					    		$("#taskbookNo").val(bhn+"沪印临"+no+"号")
					    	}
...
var edit={	editInit:function(){
	...
	var creatro1=$.trim($("#czr_1").val());
		    ...
		    dataType:"json",
			 ...
			 if ((typeof(str) == "undefined")&& (creatro1!="周志宏"||creatro1!="徐卫国")){ $(".edithight").hide();	};
			  if ((typeof(str) != "undefined")&& (creatro1!="周志宏"||creatro1!="徐卫国")){ $(".edithight").show();$("#edittr").hide();};
			 if(creatro1=="周志宏"||creatro1=="徐卫国"){ $(".edithight").show();$("#edittr").show(); }
		    	   }			
			});
