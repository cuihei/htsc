...
var taskBooks ={
	...
	init : function(){
		...
		function gotoDate(){
			...
			loading.init();
			common.init("../taskbook/list?booktype="+booktype+"&year="+year,"POST",taskBooks.bindtaskBooksGrid);
			common.do_submit();
		}
	...
	createtaskBooksColumns : function(){
		...
		grid.addColumn("180px","sjhjTime","实际汇交时间","#=sjhjTime?kendo.toString(new Date(sjhjTime),'yyyy-MM-dd'):'' #");

		grid.addColumn("70px","state","状态");
		grid.addColumn("85px","issueTime","下发时间","#=issueTime?kendo.toString(new Date(issueTime),'yyyy-MM-dd'):'' #");
		grid.addColumn("70px","ehRevision","版次");
		grid.addColumn("85px","ehCreationDate","修订时间","#=ehCreationDate?kendo.toString(new Date(ehCreationDate),'yyyy-MM-dd'):'' #");
    	grid.addColumn("145px","handle","操作",kendo.template($("#editTemplate").html()));
		grid.addColumn("100px","upload_options","附件操作",kendo.template($("#uploadTemplate").html()))
