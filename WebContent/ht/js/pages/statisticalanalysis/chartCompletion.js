...
var chartCompletion = {
	init : function(){
		...
		$("#compType").on("change",function(){
			...
			var compType = $("#compType").val();
					grid.bindServerData("../chartCompletion/all?year="+year+"&compType="+compType+"&type="+type,null);
	...
	createDistributionChartCompletionGrid : function(){
		...
		grid.bindServerData("../chartCompletion/all?year="+t+"&compType="+compType+"&type="+type,columns);
	...
	createChartCompletionGrid : function(){
		...
		grid.addLockedColumn("100px","currUser","当前操作人");

     //   grid.addColumn("100px","issueTime","任务书创建时间","#= issueTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
   
         
		grid.addColumn("100px","compilationGroup","编绘组");
		grid.addColumn("100px","compilationPerson","编绘员");
		grid.addColumn("100px","compilationWorkdays","编绘工天");
	    grid.addColumn("100px","compilationStartTime","编绘开始时间","#= compilationStartTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	    grid.addColumn("100px","compilationEndTime","编绘结束时间","#= compilationEndTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	       

		grid.addColumn("100px","qualityPerson","质检人");

		grid.addColumn("100px","qualityStartTime","质检开始时间","#= qualityStartTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	    grid.addColumn("100px","qualityEndTime","质检结束时间","#= qualityEndTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	    
		grid.addColumn("100px","qualityScore","质检评分");
		grid.addColumn("100px","checkWorkdays","质检工天");
	    grid.addColumn("100px","ztTime","制图事业部科长审核时间","#= ztTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	   
		grid.addColumn("100px","apprvoePerson","审定人");
		grid.addColumn("100px","apprvoeStartTime","审定开始时间","#= apprvoeStartTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	    grid.addColumn("100px","apprvoeEndTime","审定结束时间","#= apprvoeEndTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
	    grid.addColumn("100px","examineWorkdays","审定工天");
		grid.addColumn("100px","score","审定评分");
		grid.addColumn("100px","zlTime","质量检验科科长审核时间","#= zlTime ? kendo.toString(new Date(createTime), 'yyyy-MM-dd HH:mm:ss') : '' #" );
		...
		grid.bindServerData("../chartCompletion/all?compType="+compType+"&year="+year+"&type="+type,columns);
	    }else{
			grid.bindServerData("../chartCompletion/listByKey?processDefKey="+key+"&isComplete="+isComplete+"&type="+type,columns);
		}
	...
	requestChartCompletionData : function(){
		...
		if(key==null || key=='' || key==undefined){
			...
			common.init("../chartCompletion/all?year="+t+"&type="+type,"POST",chartCompletion.bindChartCompletionGrid);
		
		}else{
	
			common.init("../chartCompletion/listByKey?processDefKey="+key+"&isComplete="+isComplete,"POST",chartCompletion.bindChartCompletionGrid);
