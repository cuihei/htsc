$(function() {
	...
	/*		grid.addColumn("200px", "opinion", "存在问题及处理意见");
		...
		grid.addColumn("200px", "discription", "缺陷内容");
...
var defect_form = {
	...
	init : function() {
		...
		common.init("../defect/list", "POST", function(result) {
		...
		function onChange() {
			...
			if (defecttypeId == null || defecttypeId == "") {
				...
				common.init("../defectitem/item?defecttypeId=" + defecttypeId,
			...
			common.init("../defect/content?charttypeId=" + charttypeId
					+ "&defecttypeId=" + defecttypeId, "POST",
					function(result) {
						// 绑定缺陷数据源
						var dataSource = new kendo.data.DataSource({
							data : result.value
						});
						var dropdownlist = $("#discription").data(
								"kendoDropDownList");
						dropdownlist.setDataSource(dataSource);
					});
			common.do_submit(param);
		...
		function onChangeDefect() {
			...
			if (id == null || id == "") {
				...
				common.init("../defect/defect?id=" + id, "POST", function(
						result) {
					var obj = result.value;
					if (obj == undefined) {
						return;
					...
					var itemDropdownlist = $("#defectitem").data(
							"kendoDropDownList");
	...
	requestData : function() {
		...
		var processDefId = $("#processDefId").val();
		var taskDefId = $("#taskDefId").val();
		common.init("../defect/formlist?taskDefId=" + taskDefId
				+ "&processDefId=" + processDefId + "&splitId=" + splitId
				+ "&taskInstId=" + taskInstId + "&processInstId="
				+ processInstId, "POST", defect_form.bindGrid);
		common.do_submit();
	},
	...
	calculate : function() {
		...
		if (level == "Ⅰ") {
			...
			score = 0.5;
			actual = (0.5).toFixed(1);
		}
		var sum = score;
		var actualSum = actual;
		if (num != null && num != "") {
			sum = num * (score);
		}
		$("#score").val(sum);
		/*
		 * $("#actual").val(parseFloat(actualSum).toFixed(1)); // 获取累计得分 var
		 * zong = (parseFloat(aggregate)+parseFloat(sum)).toFixed(1);
		 * $("#total").val(parseFloat(zong)); var shi =
		 * (parseFloat(fact)+parseFloat(actualSum)).toFixed(1);
		 * $("#actualTotal").val(parseFloat(shi)); $("#grading").val(100-shi);
		 */
	},
	...
	requestPlanData : function() {
		...
		common.add_param("taskDefId", taskDefId);
		var param = common.add_param("parentProcessInstId", $(
				"#parentProcessInstId").val());
		common.init("../formValue/getByProcess", "POST",
				defect_form.bindPlanGrid);
		common.do_submit(param);
	},
	...
	bindPageEvent : function() {
		...
		$("#submit").on("click", function() {
			...
			var defecttypecolor = $("#defecttype").data("kendoDropDownList");
			var type = defecttypecolor.text();
			if (type == "请选择......") {
				return layer.msg("请选择类别！");
			}
			defect.defecttype = type;
			var defectitemcolor = $("#defectitem").data("kendoDropDownList");
			var item = defectitemcolor.text();
			if (item == "请选择......") {
				return layer.msg("请选择检查项目！");
			}
			defect.defectitem = item;
			...
			/*
			 * if(Number(score)<=0||Number(score)>=100){ return
			 * layer.msg("扣分必须在0-100之间"); } var grading = $("#grading").val();
			 * if(Number(grading)<=0||Number(grading)>=100){ return
			 * layer.msg("质量评分必须在0-100之间"); }
			 */
			defect.score = score;
			// 实际得分
		...
		$("#back").on(
				...
				function() {
					...
					defect.taskInstId = $("#taskInstId").val();
					defect.processInstId = $("#processInstId").val();
					defect.defaultValue = $("#defaultValue").val();
					var processDefId = $("#processDefId").val();
					var taskDefId = $("#taskDefId").val();
					common.add_param("processDefId", processDefId);
					common
							.add_param("processInstId", $("#processInstId")
									.val());
					common.add_param("taskDefId", taskDefId);
					common.add_param("taskInstId", $("#taskInstId").val());
					var defectJson = JSON.stringify(defect);
