...
var hi_task = {
		...
		init : function(){
			...
			try{
				// 创建表格
				hi_task.createProcessInstGrid();
				// 发送数据请求绑定表格数据
				hi_task.requestProcessInstData();
			}
			catch(err){
				loading.close();
			}
			...
			function changeYearMonth(){
				var date = $("#year").val();
				loading.init();
				common.init("../workflow/hi_task_list_index","POST",hi_task.bindRuTaskGrid);
		...
		show : function(obj){
			...
			common.toPage("../workflow/hi_task_init_index?processDefKey="+processDefKey+"&processInstId="+procInstId+"&parentProcessInstId="+parentProcessInstId);
		},
		/**
		 * 创建任务表格
		 */
		 ...
		 * 创建任务表格
		 */
		createProcessInstGrid : function(){
			var processApprove  = ["CATALOG_MANAGEMENT","TASK_BOOK","DATA_INPUT","DATA_BORROWING","DATA_RETURN"];
			var taskDef = "# var processSubmitApprove = ['u_task_plan_zhijian_shenhe','u_task_ztkz_queren','u_task_ztkz_shenhe'];" +
			"if($.inArray(taskDefId,processSubmitApprove)<0){#";
			var approveHTML= $('#operation').html();
			...
			var columns = grid.addColumn("40px","flow","查看详情",kendo.template($("#show").html()));
			grid.createGrid(columns);
		},
		/**
		 * 发送获取任务列表请求
		 */
		requestProcessInstData : function(){
			common.init("../workflow/hi_task_list_index","POST",hi_task.bindRuTaskGrid);
			var processDefKey = $("#processDefKey").val();
			var date = $("#year").val();
			common.add_param("year",date);
