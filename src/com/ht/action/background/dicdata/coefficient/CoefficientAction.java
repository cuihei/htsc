...
public class CoefficientAction extends BaseAction {
	 ...
	 * 
	 */
	public void mapNoexi() {
		try {
		String mapno = getParam("mapno");
	    List<String> mapnolist=coefficientService.mapNoexi(mapno);
		writeSuccessResult(mapnolist);
	...
	public void addCoe() {
		try {
			String coe = getParam("coe");
			   JSONObject a = new JSONObject(coe); 
			   String mapno =a.getString("mapNo"); 
			   String id =a.getString("id"); 
			   JSONObject b = new JSONObject(a.getString("type")); 
			   String typeid = b.getString("id"); 
			   String firstEdition=a.getString("firstEdition");
			   String reprint=a.getString("reprint");
			  int mapcount=coefficientService.mapcount(mapno,typeid);

		      coefficientService.add(coe);
		      writeSuccessResult(1); 
			  
/*	       if(firstEdition.length()>0&&reprint.length()>0&&mapcount<1)
	       {
           coefficientService.add(coe);
  
	      
	       if((mapcount>0&&typeid.indexOf("11061935001990001")>-1) ||(typeid.indexOf("11061935001990001")>-1))
	       {
    		   writeSuccessResult(1); 
	       }
