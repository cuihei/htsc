...
public class TaskBookAction extends BaseAction {
	...
	public String index() {
		...
		try {
			...
			String html = page.getListPage(taskBookService, taskBookType, baseDataService, jurisdiction);
	...
	public void getTaskBookList() {
		String booktype = getParam("booktype");//添加任务书类型 0全部  1指令性 2临时性
		...
		try {
			...
			jurisList.add("11031903128270101");
			boolean jurisdiction = this.getUserJurisdiction(roleList, jurisList);
			List<TaskBookList> booklist = taskBookService.findList(year, jurisdiction,booktype);
	...
	public void selectPlan() {
		try {
			...
			/*
			 * if (list != null) { for (int i = 0; i < list.size(); i++) { Plan plan =
			 * list.get(i).getPlan(); if (plan != null) {
			 * plan.setRevision(list.get(i).getRevision()); planList.add(plan); } } }
			 */
	...
	public void update() {
		try {
			...
			if (StringUtils.isNotEmpty(planId)) {
				...
				///给TASKBOOK的新增的任务书ID赋值，以保证指令性任务与书可以同步 2019.8.16
				taskBook.setPlanId(planId);
			}
			taskBookService.modifyTaskBook(taskBook, planIds, revisions);
			/*
			 * taskBookService.modifyTaskBook(taskBook, planIds, revisions, processInstId,
			 * taskId, userNo);
			 */ writeSuccessResult();
		} catch (Exception e) {
			writeFailResult(e.getMessage());
	...
	public String editPage() throws Exception {
		...
		String html = page.getEditPage(taskBookType, taskBookService, organizationService, id, processInstId, taskId,
				processDefId, taskDefId, vProcessDetailDao, taskBookPlanRelationService, baseDataService, flag);
		request.setAttribute("html", html);
	...
	public void Export() throws Exception {
		...
		try {
			...
			if (StringUtils.isNotEmpty(id)) {
				...
				response.setHeader("Content-Disposition", "attachment;fileName=" + bookName + ".doc");
				// 获取计划列表
				List<TaskBill> list = taskBillService.findListByTaskBookId(taskbook.getId());
				List<Plan> planList = new ArrayList<Plan>();
				if (list != null) {
				...
				if (null != executeDeptId && !"".equals(executeDeptId)) {
					...
					for (String string : executeDeptIdList) {
						Organization organization = organizationService.getOrganization(string);
						if (organization != null) {
							if (executeDept == null && !"".equals(executeDept)) {
								executeDept = organization.getOrgName();
							} else {
								executeDept = executeDept + "," + organization.getOrgName();
							}
						}
					}
				}
				...
				if (taskbook.getExecuteTime() == null) {
					...
					map.put("text_execute_time", sdf.format(taskbook.getExecuteTime()));
				}
				...
				String serverPath = request.getServletContext().getRealPath("/");
				// 获取模版
				File templete = new File(serverPath + "/ht/templete/taskbookTemplete.docx");
	 ...
	 * 任务书编号获取最大值
	 */
	public void taskselect() throws Exception {
		String taskselect = getParam("taskselect");

		String taskNUM = taskBookService.findTaskselect(taskselect);


		writeSuccessResult(taskNUM);
	}
