package com.ht.front.pages.complication.correctionnotice.sourcedatasmallcorrection;
...
public class SourSmallCorPage {
	...
	public String getStatusPage() {
		FrontUtil util = FrontUtil.getInstance();
		Base root  = util.createRoot();
		util.createHeaderBar(root, "源数据小改正流转状态查看");
		// 创建行
		Base rowGrid = util.createRow();
		// 创建列
		Base columnGrid = util.createColumn(rowGrid, "12","12");
		// 创建Grid
		util.createGrid(columnGrid,"sourSmallCorStatus");
		root.addChildNode(columnGrid);
		root.addChildNode(rowGrid);
		//返回节点字符串
	...
	public String getDetailsPage(String id) {
		...
		A a = A.getInstance(href,"小改正资料登记表");
		...
		a = A.getInstance(href,"源数据小改正问题处理记录表");	
		h4 = Templete.getInstance("h4",h4Css);
		h4.addChildNode(pointDiv);
		h4.addChildNode(a);
		div = Div.getInstance(null,css, null);
		...
		href="../source/index";
		a = A.getInstance(href,"源数据小改正质检记录表");	
		h4 = Templete.getInstance("h4",h4Css);
		h4.addChildNode(pointDiv);
		h4.addChildNode(a);
		div = Div.getInstance(null,css, null);
		...
		href="../source/index";
		a = A.getInstance(href,"源数据小改正审定记录表");	
		h4 = Templete.getInstance("h4",h4Css);
		h4.addChildNode(pointDiv);
		h4.addChildNode(a);
		div = Div.getInstance(null,css, null);
