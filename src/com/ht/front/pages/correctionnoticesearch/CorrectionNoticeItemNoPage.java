...
public class CorrectionNoticeItemNoPage{

	FrontUtil util = null;
	
	Base root = null;
	
	public CorrectionNoticeItemNoPage(){
		// 获取前端工具实例
	...
	public String getPage(CorrectionNoticeService correctionNoticeService,String gridName){
		...
		util.createHeaderBar(root, "改正通告项号查询");
		//创建搜索模块
	...
	private void createSearchPage(CorrectionNoticeService correctionNoticeService){
		...
		// 创建列
		Base col1 = util.createColumn(row,"2","2", "2",null);
		// 创建列
		Base col2 = util.createColumn(row,"2","2", "2",null);
		// 创建列
		Base col3 = util.createColumn(row,"4","4", "4",null);
		// 创建列
		Base col4 = util.createColumn(row,"2","2", "2",null);
		// 创建列
		Base col5 = util.createColumn(row,"2","2", "2",null);
		
		//创建开始时间搜索框
	/*	InputGroup ig = (InputGroup) util.createRangeDatePickerGroup("创建时间", "startTime", "请输入创建时间", "endTime", "请输入创建时间");
		...
		for (int i = 0; i <yearList.size(); i++) {
			...
			String id = String.valueOf(yearList.get(i).getID());
			String year = yearList.get(i).getName();
			b.setID(id);
			b.setName(year);
			list.add(b);
		}
		...
		//创建项号搜索框
		InputGroup mapNumInput = InputGroup.getInGroup("项&#12288&#12288号", "itemNo",null, "请输入项号");
		col2.addChildNode(mapNumInput);
		//创建关键字搜索框
		InputGroup keyInput = InputGroup.getInGroup("关键字&#12288", "key",null, "请输入关键词,使用空格分隔");
		col3.addChildNode(keyInput);
		/*//添加1个行间距
		root.addChildNode(util.createRowSpace());
		// 创建第二行
		row = util.createRow(root);
		// 创建列
		col1 = util.createColumn(row,"4","4", "4",null);
		// 创建列
		col2 = util.createColumn(row,"4","4", "4",null);
		// 创建列
		col3 = util.createColumn(row,"4","4", "4",null);*/
		// 创建列
		//创建通告类型搜索框
