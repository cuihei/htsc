package com.ht.front.pages.datum.bookinfo;
...
public class BookInfoPage {
	 ...
	 * @param datumcategoryService
	 * @param
	 * @return 节点字符串
	 */
	public String getListNode(BaseDataService baseDataService,
			DatumCategoryService datumcategoryService, List<String> ids,List<OrganizationUsersRelation> result,UserService userService){
		...
		util.createHeaderBar(root, "图书资料");
		...
		// 创建列
		Base col3 = util.createColumn(row, "6", "6", "6", null);

		// 创建一级子类下拉框搜索框
		List<DatumCategory> dcList = datumcategoryService
				.getDatumCategoryByParentId("201610301150");
		Base onesubclass = util.creatDefaultSelectGroup("一级子类", "oneSubClass",
				dcList, "id", "categoryName", false);
		col1.addChildNode(onesubclass);
		// 创建二级子类下拉框搜索框
		Base twosubclass = util.creatDefaultSelectGroup("二级子类", "twoSubClass",
				null, "id", "categoryName", false);
		col2.addChildNode(twosubclass);

		// 创建出版年月搜索框
		InputGroup ig = (InputGroup) util.createYearMonthDatePickerGroup(
				"出版日期", "publishDate", "请输入出版日期", "publishDateTwo", "请输入出版日期");
		...
		// 创建列
		col1 = util.createColumn(row, "3", "3", "3", null);
		// 创建列
		col2 = util.createColumn(row, "3", "3", "3", null);
		// 创建列
		col3 = util.createColumn(row, "6", "6", "6", null);
		// 创建状态下拉框
		List<BaseData> stateList = baseDataService
				.getBaseDataByTypeId("11031952486860383");
		Base state = util.creatDefaultSelectGroup("状&#12288&#12288态", "state",
				stateList, "id", "value", false);
		col1.addChildNode(state);

		// 创建名称搜索框
		InputGroup mapNumInput = InputGroup.getInGroup("模糊查询", "name", null,
				"请输关键字");
		col2.addChildNode(mapNumInput);

		// 创建搜索按钮
		Button searchButton = Button.getButtonWithIcon("search", new CssClass(
				"btn btn-primary search"), "查询", I.getInstance(new CssClass(
				"fa fa-search")));
		col3.addChildNode(searchButton);

		/*
		 * / 创建版本号搜索框 InputGroup keyInput = InputGroup.getInGroup("版本号&#12288",
		 * "version", null, "请输入版本号"); col2.addChildNode(keyInput); // 创建出版单位搜索框
		 * keyInput = InputGroup .getInGroup("出版单位", "publishUnit", null,
		 * "请输入出版单位"); col3.addChildNode(keyInput);
		 */

		/*
		 * / 添加1个行间距 root.addChildNode(util.createRowSpace()); // 创建第二行 row =
		 * util.createRow(root); // 创建列 col1 = util.createColumn(row, "6", "6",
		 * "6", null); // 创建列 col2 = util.createColumn(row, "6", "6", "6",
		 * null);
		 */
		...
		if (ids.contains("11031915039750121")) {// 资料录入员能看到所有
			...
			Button button = Button.getButtonWithIcon("add", css, "&nbsp;"
					+ "创建", i);
			...
			button = Button
					.getButtonWithIcon("remove", css, "&nbsp;" + "删除", i);
			column.addChildNode(button);
			// 构建导入按钮
			...
			button = Button
					.getButtonWithIcon("import", css, "&nbsp;" + "导入", i);
			column.addChildNode(button);
			// 构建导出div
			...
			i = I.getInstance(css);
			css = new CssClass("btn btn-info bk-margin-5 search");
			button = Button.getButtonWithIcon("batchBorrowing", css, "&nbsp;"
					+ "借阅", i);
			column.addChildNode(button);
			// 构建提交审核按钮
			...
			I i = I.getInstance(css);
			css = new CssClass("btn btn-info bk-margin-5 search");
			Button button = Button.getButtonWithIcon("batchBorrowing", css,
					"&nbsp;" + "借阅", i);
			column.addChildNode(button);
		}
		...
		/** 创建Grid行 开始 */
		// 创建Grid
		Base rowGrid = util.createGrid(root, "bookInfo");
		/** 创建Grid行 结束 */
		// 添加操作按钮
		// 上传
		...
		if (ids.contains("11031915039750121")) {// 资料录入员能看到所有
			...
			uploadTempelate = Button.getButtonWithIcon(null, uploadCss, null,
					uploadI);
			...
			prop.setPropValue("editBookInfo");
		...
		// 标识隐藏
		InputHidden bookInfoIdHidden = InputHidden.getInstance("bookInfoId",
				null);
		Prop bookInfoIdProp = new Prop();
		bookInfoIdProp.setPropKey("name");
		bookInfoIdProp.setPropValue("bookInfoId");
		...
		// 创建标题div
		CssClass titleCss2 = new CssClass(
				"modal-title bk-fg-primary model-custom");
		...
		Button tempBtn2 = Button.getInstance("exportInfoTemplate", tempCss2,
				"下载模板");
		inputDiv2.addChildNode(tempBtn2);
		// 构建导入按钮
		CssClass importCss2 = new CssClass("import-submit btn btn-primary");
		Button importBtn2 = Button.getInstance("importInfoSubmit", importCss2,
				"导入");
	 ...
	 * @return
	 */
	public String getEditNode(String id, BookInfoService bookInfoService,
			BaseDataService baseDataService, UserService userService,
			List<DatumCategory> list, DatumCategoryService datumcategoryService) {
		...
		try {
			if (id != null) {
				...
				InputGroup tb = InputGroup.getInGroup("图书编号", "code",
						bookInfo.getCode(), "请输入图书编号");
				param.add(tb);
				tb = InputGroup.getInGroup("图书名称", "bookName",
						bookInfo.getBookName(), "请输入图书名称");
				param.add(tb);
				tb = InputGroup.getInGroup("图书作者", "author",
						bookInfo.getAuthor(), "请输入图书作者");
				param.add(tb);
				Base selectgroup = util.creatDefaultSelectGroup("一级子类",
						"oneSubClass", list, "id", "categoryName",
						bookInfo.getOneSubClass());
				param.add(selectgroup);
				List<DatumCategory> twoList = datumcategoryService
						.getDatumCategoryByParentId(bookInfo.getOneSubClass());
				selectgroup = util.creatDefaultSelectGroup("二级子类",
						"twoSubClass", twoList, "id", "categoryName",
						bookInfo.getTwoSubClass());
				param.add(selectgroup);
				tb = InputGroup.getInGroup("图书书号", "bookNo",
						bookInfo.getBookNo(), "请输入图书书号");
				param.add(tb);
				tb = InputGroup.getInGroup("出版单位", "publishUnit",
						bookInfo.getPublishUnit(), "请输入出版单位");
				param.add(tb);
				// 获取出版年月
				Date date = bookInfo.getPublishDate();
				if (date != null) {
					String publishDate = (new SimpleDateFormat("yyyy-MM"))
							.format(date);
					tb = InputGroup.getDatePickerYearMonth("出版年月",
				...
				String version = bookInfo.getVersion();
				if (version != null && version != "") {
					tb = InputGroup.getInGroup("版本号&#12288", "version",
							version, "请输入版本号");
					param.add(tb);
				} else {
					tb = InputGroup.getInGroup("版本号&#12288", "version", null,
							"请输入版本号");
					param.add(tb);
				}

				// 获取印刷年月
				date = bookInfo.getPrintDate();
				if (date != null) {
					String printDate = (new SimpleDateFormat("yyyy-MM"))
							.format(date);
					tb = InputGroup.getDatePickerYearMonth("印刷年月", "printDate",
							printDate, null);
					param.add(tb);
				} else {
					tb = InputGroup.getDatePickerYearMonth("印刷年月", "printDate",
				...
				tb = InputGroup.getInGroup("图书定价", "price",
						bookInfo.getPrice(), "请输入图书定价");
				param.add(tb);
				tb = InputGroup.getInGroup("库存总数", "total",
						bookInfo.getTotal(), "请输入库存总数");
				param.add(tb);
				tb = InputGroup.getInGroup(true, "在库数量", "inventoryNum",
						bookInfo.getInventoryNum(), "请输入在库数量");
				param.add(tb);
				tb = InputGroup.getInGroup(true, "可借数量", "canBorrowing",
						bookInfo.getCanBorrowing(), "请输入可借数量");
				param.add(tb);
				tb = InputGroup.getInGroup("存储位置", "savePlace",
						bookInfo.getSavePlace(), "请输入存储位置");
				param.add(tb);
				/*
				 * tb = InputGroup.getInGroup("资料状态", "fileStatus",
				 * bookInfo.getFileStatus(), "请输入资料状态"); param.add(tb); tb =
				 * InputGroup.getInGroup("图书状态", "state", bookInfo.getState(),
				 * "请输入图书状态"); param.add(tb);
				 */
				// 电子数据下载权限
				tb = InputGroup.getInGroup("下载权限", "downPermission",
						bookInfo.getDownPermission(), "请输入电子数据下载权限");
				param.add(tb);
				// param.add(null);
				// 备注
				InputGroup textArea = InputGroup.getTextAreaGroup(
						"备&#12288&#12288注",
						"remarks",
						"5",
						bookInfo.getRemarks() == null ? null : bookInfo
								.getRemarks());
				param.add(textArea);
				// 流转意见
				InputGroup flowSuggestion = InputGroup.getTextAreaGroup(
						"流转意见",
						"flowSuggestion",
						"5",
						bookInfo.getFlowSuggestion() == null ? null : bookInfo
								.getFlowSuggestion());
				param.add(flowSuggestion);
				editPage = edit.createEditPage(param);
				util.createHeaderBar(editPage, "图书资料编辑");
				InputHidden hidden = InputHidden.getInstance("bookInfoId", id);
				editPage.addChildNode(hidden);
				hidden = InputHidden.getInstance("sum", bookInfo.getTotal());
				editPage.addChildNode(hidden);
			} else {
				List<Base> param = new ArrayList<Base>();
				InputGroup tb = InputGroup.getInGroup("图书编号", "code", null,
						"请输入图书编号");
				param.add(tb);
				tb = InputGroup.getInGroup("图书名称", "bookName", null, "请输入图书名称");
				param.add(tb);
				tb = InputGroup.getInGroup("图书作者", "author", null, "请输入图书作者");
				param.add(tb);
				Base selectgroup = util.creatDefaultSelectGroup("一级子类",
						"oneSubClass", list, "id", "categoryName", false);
				param.add(selectgroup);
				selectgroup = util.creatDefaultSelectGroup("二级子类",
						"twoSubClass", null, "id", "categoryName", false);
				param.add(selectgroup);
				tb = InputGroup.getInGroup("图书书号", "bookNo", null, "请输入图书书号");
				param.add(tb);
				tb = InputGroup.getInGroup("出版单位", "publishUnit", null,
						"请输入出版单位");
				param.add(tb);
				tb = InputGroup.getDatePickerYearMonth("出版年月", "publishDate",
						null, null);
				param.add(tb);

				tb = InputGroup.getInGroup("&#12288版本号", "version", null,
						"请输入版本号");
				param.add(tb);
				// 获取印刷年月
				tb = InputGroup.getDatePickerYearMonth("印刷年月", "printDate",
						null, null);
				param.add(tb);
				tb = InputGroup.getInGroup("图书定价", "price", null, "请输入图书定价");
				param.add(tb);
				tb = InputGroup.getInGroup("库存总数", "total", null, "请输入库存总数");
				param.add(tb);
				tb = InputGroup
						.getInGroup("存储位置", "savePlace", null, "请输入存储位置");
				param.add(tb);
				/*
				 * tb = InputGroup.getInGroup("资料状态", "fileStatus",null,
				 * "请输入资料状态"); param.add(tb); tb = InputGroup.getInGroup("图书状态",
				 * "state", null, "请输入状态"); param.add(tb);
				 */
				// 电子数据下载权限
				tb = InputGroup.getInGroup("下载权限", "downPermission", null,
						"请输入电子数据下载权限");
				param.add(tb);
				// param.add(null);
				// 备注
				InputGroup textArea = InputGroup.getTextAreaGroup(
						"备&#12288&#12288注", "remarks", "5", null);
				param.add(textArea);
				// 流转意见
				InputGroup flowSuggestion = InputGroup.getTextAreaGroup("流转意见",
						"flowSuggestion", "5", null);
				param.add(flowSuggestion);
				editPage = edit.createEditPage(param);
				util.createHeaderBar(editPage, "图书资料创建");
			}
			// 返回节点字符串
	 ...
	 */
	/*
	 * public String getFileListNode(String bookId){ // 创建前端工具实例 FrontUtil util
	 * = FrontUtil.getInstance(); // 创建一个容器 Base root = util.createRoot();
	 * util.createHeaderBar(root, "资料附件管理");
	 *//** 创建按钮组行 开始 */
	/*
	 * // 创建行 Base rowBg = util.createRow(); // 创建列 Base column =
	 * util.createColumn(rowBg, "12","12"); // 创建按钮组 // 构建行div CssClass css =
	 * new CssClass("row"); util.createRow(column); css = new
	 * CssClass("col-md-12 col-xs-12 col-sm-12"); Div div =
	 * Div.getInstance(null, css, null); // 构建创建div css = new
	 * CssClass("fa fa-mail-reply-all"); I i = I.getInstance(css); css = new
	 * CssClass("btn btn-success "); Button button =
	 * Button.getButtonWithIcon("back", css, "返回", i); div.addChildNode(button);
	 * column.addChildNode(div);
	 *//** 创建按钮组行 结束 */
	/*
	 * // 创建一个行间隔 Base rowSpace = util.createRowSpace();
	 *//** 创建Grid行 开始 */
	/*
	 * // 创建行 Base rowGrid = util.createRow(); // 创建列 Base columnGrid =
	 * util.createColumn(rowGrid, "12","12"); // 创建Grid
	 * util.createGrid(columnGrid,"bookfile");
	 *//** 创建Grid行 结束 */
	/*
	 * // 将行加入到容器 root.addChildNode(rowBg); root.addChildNode(rowSpace);
	 * root.addChildNode(rowGrid); // 添加编辑按钮 CssClass editCss = new
	 * CssClass("fa fa-cloud-download"); I editI = I.getInstance(editCss);
	 * editCss = new CssClass("btn btn-success bk-margin-5"); Button tempelate =
	 * Button.getButtonWithIcon(null, editCss, null, editI); Prop prop = new
	 * Prop(); prop.setPropKey("name"); prop.setPropValue("downloadFile");
	 * tempelate.addProp(prop); Script script =
	 * Script.getInstance("editTemplate"); script.addChildNode(tempelate);
	 * 
	 * InputHidden hidden = InputHidden.getInstance("bookId", bookId);
	 * root.addChildNode(hidden); return root.getNode() + script.getNode(); }
	 */
	 ...
	 * @return
	 */
	public String getBorrowing(BookInfo bookInfo) {
		EditPage edit = new EditPage();
		FrontUtil util = FrontUtil.getInstance();
		List<Base> param = new ArrayList<Base>();
		CssClass Css = new CssClass("form-control");
		...
		TextBox borrowBookNoTb = TextBox.getInstance("code", "code",
				bookInfo.getCode(), Css, "请输入图书编号 ");
		...
		TextBox borrowBookNameTb = TextBox.getInstance("borrowBookName",
				"borrowBookName", bookInfo.getBookName(), Css, "请输入图书编号 ");
		borrowBookNameTb.addProp(prop);
		param.add(InputGroup.getInstance(borrowBookNameSpan, borrowBookNameTb));

		// 设置图书名称输入框
		/*
		 * String borrowPerson = "借阅者"; Span borrowPersonSpan =
		 * Span.getDefault(borrowPerson); TextBox borrowPersonTb =
		 * TextBox.getInstance("borrowPerson", "borrowPerson", null, Css,
		 * "请输入借阅者 "); param.add(InputGroup.getInstance(borrowPersonSpan,
		 * borrowPersonTb));
		 */

		/*
		 * // 设置借阅日期 InputGroup tb =
		 * InputGroup.getDatePicker("借阅日期"+"&nbsp;&nbsp;", "borrowDate", null,
		 * null); param.add(tb);
		 */
		...
		TextBox canBorrowNoTb = TextBox.getInstance("canBorrowNo",
				"canBorrowNo", bookInfo.getCanBorrowing(), Css, "请输入借阅数量 ");
		...
		TextBox borrowNoTb = TextBox.getInstance("borrowNo", "borrowNo", null,
				Css, "请输入借阅数量 ");
		param.add(InputGroup.getInstance(borrowNoSpan, borrowNoTb));

		/*
		 * // 设置借阅天数 String borrowDays = "借阅天数"; Span borrowDaysSpan =
		 * Span.getDefault(borrowDays); TextBox borrowDaysTb =
		 * TextBox.getInstance("borrowDays", "borrowDays", null, Css,
		 * "请输入借阅天数 "); param.add(InputGroup.getInstance(borrowDaysSpan,
		 * borrowDaysTb));
		 */
		...
		Base selectgroup = util.creatDefaultSelectGroup("资料是否涉密", "isSecrecy",
				secrecylist, "id", "value", false);
		param.add(selectgroup);

		Base editPage = edit.createEditPage(param);
		util.createHeaderBar(editPage, "图书借阅");
		InputHidden hidden = InputHidden
				.getInstance("bookId", bookInfo.getId());
		editPage.addChildNode(hidden);
		// 返回节点字符串
		return editPage.getNode();
	}
	 ...
	 * @return
	 */
	public String getBatchRerurn(String ids) {

		// 创建前端工具实例
		FrontUtil util = FrontUtil.getInstance();
		// 创建一个容器
		Base root = util.createRoot();
		util.createHeaderBar(root, "资料归还申请表");
		util.createRowSpace(root);
		/** 创建按钮组行 开始 */
		// 创建行
		Base rowBg = util.createRow(root);
		...
		/** 创建Grid行 开始 */
		// 创建Grid
		Base rowGrid = util.createGrid(root, "batchReturn");
		/** 创建Grid行 结束 */
		InputHidden hidden = InputHidden.getInstance("ids", ids);
		root.addChildNode(hidden);
		return root.getNode();
	 ...
	 * @return
	 */
	public Object getReturn(BookInfo bookInfo) {
		EditPage edit = new EditPage();
		FrontUtil util = FrontUtil.getInstance();
		List<Base> param = new ArrayList<Base>();
		...
		String borrowBookNo = "图书编号";
		Span borrowBookNoSpan = Span.getDefault(borrowBookNo);
		TextBox borrowBookNoTb = TextBox.getInstance("BookNo", "BookNo",
				bookInfo.getId(), Css, "请输入图书编号 ");
		borrowBookNoTb.addProp(prop);
		param.add(InputGroup.getInstance(borrowBookNoSpan, borrowBookNoTb));
		...
		String borrowBook = "图书名称";
		Span borrowBookNameSpan = Span.getDefault(borrowBook);
		TextBox borrowBookNameTb = TextBox.getInstance("BookName", "BookName",
				bookInfo.getBookName(), Css, "请输入图书编号 ");
		borrowBookNameTb.addProp(prop);
		param.add(InputGroup.getInstance(borrowBookNameSpan, borrowBookNameTb));
		...
		TextBox returnPersonTb = TextBox.getInstance("returnPerson",
				"returnPerson", null, Css, "请输入归还人 ");
		param.add(InputGroup.getInstance(returnPersonSpan, returnPersonTb));

		// 设置归还日期
		InputGroup tb = InputGroup.getDatePicker("归还日期" + "&nbsp;&nbsp;",
		...
		TextBox returnNoTb = TextBox.getInstance("returnNo", "returnNo", null,
				Css, "请输入归还数量 ");
		...
		TextBox borrowDaysTb = TextBox.getInstance("borrowDays", "borrowDays",
				null, Css, "请输入借阅天数 ");
	...
	public String getFileListNode(UserService userService,
			BaseDataService baseDataService, BookInfoService bookInfoService,
			BookFileService bookFileService, String id,
			List<DatumCategory> list, String mark, boolean flag)
			throws Exception {
		List<Base> param = new ArrayList<Base>();
		// 获取图书
		BookInfo bookInfo = bookInfoService.getBookInfo(id);
		...
		// 创建一个容器
		Base root = util.createRoot();
		util.createHeaderBar(root, "航海图书信息查看");
		util.createRowSpace(root);
		// 创建第一行
		Base row = util.createRow(root);
		// 创建列
		...
		// 编码
		InputGroup input = InputGroup.getInGroup(true, "图书编号", "code",
				bookInfo.getCode(), null);
		col1.addChildNode(input);
		// 名 称
		input = InputGroup.getInGroup(true, "图书名称", "bookName",
				bookInfo.getBookName(), null);
		col2.addChildNode(input);
		// 作 者
		input = InputGroup.getInGroup(true, "图书作者", "author",
				bookInfo.getAuthor(), null);
		col3.addChildNode(input);
		// 书 号
		input = InputGroup.getInGroup(true, "图书书号", "bookNo",
				bookInfo.getBookNo(), null);
		...
		// 一级子类
		Base selectgroup = util.creatDefaultSelectGroup("一级子类", "oneSubClass",
				list, "id", "categoryName", bookInfo.getOneSubClass());
		col1.addChildNode(selectgroup);
		selectgroup = util.creatDefaultSelectGroup("二级子类", "twoSubClass", list,
				"id", "categoryName", bookInfo.getTwoSubClass());
		col2.addChildNode(selectgroup);
		// 出版单位
		input = InputGroup.getInGroup(true, "出版单位", "publishUnit",
				bookInfo.getPublishUnit(), null);
		col3.addChildNode(input);
		// 出版日期
		String publishDate = "";
		if (bookInfo.getPublishDate() != null) {
			publishDate = new SimpleDateFormat("yyyy-MM").format(bookInfo
					.getPublishDate());
		}
		input = InputGroup.getInGroup(true, "出版年月", "publishDate", publishDate,
				null);
		col4.addChildNode(input);
		...
		// 版本号
		input = InputGroup.getInGroup(true, "&#12288版本号", "version",
				bookInfo.getVersion(), null);
		col1.addChildNode(input);
		// 印刷年月
		String printDate = "";
		if (bookInfo.getPrintDate() != null) {
			printDate = (new SimpleDateFormat("yyyy-MM")).format(bookInfo
					.getPrintDate());
		}
		input = InputGroup.getInGroup(true, "印刷年月", "printDate", printDate,
				null);
		col2.addChildNode(input);
		// 定价
		input = InputGroup.getInGroup(true, "图书定价", "price",
				bookInfo.getPrice(), null);
		col3.addChildNode(input);
		// 库存数量
		input = InputGroup.getInGroup(true, "库存数量", "inventoryNum",
				bookInfo.getInventoryNum(), null);
		col4.addChildNode(input);
		...
		// 存储位置
		input = InputGroup.getInGroup(true, "存储位置", "savePlace",
				bookInfo.getSavePlace(), null);
		col1.addChildNode(input);
		// 录入日期
		String date = "";
		if (bookInfo.getCreationDate() != null) {
			date = (new SimpleDateFormat("yyyy-MM")).format(bookInfo
					.getCreationDate());
		}
		input = InputGroup.getInGroup(true, "录入日期", "creationDate", date, null);
		col2.addChildNode(input);
		...
		// 资料状态
		input = InputGroup.getInGroup(true, "资料状态", "fileStatus",
				bookInfo.getFileStatus(), null);
		col1.addChildNode(input);
		List<BaseData> baseDataById = baseDataService.getBaseDataById(bookInfo
				.getState());
		String state = "";
		if (baseDataById != null && baseDataById.size() > 0) {
			state = baseDataById.get(0).getValue();
		}
		// 状 态
		input = InputGroup.getInGroup(true, "图书状态", "state", state, null);
		col2.addChildNode(input);
		// 电子数据下载权限
		input = InputGroup.getInGroup(true, "下载权限", "downPermission",
				bookInfo.getDownPermission(), null);
		col3.addChildNode(input);
		...
		// 备注
		input = InputGroup.getInGroup(true, "备&#12288&#12288注", "remarks",
				bookInfo.getRemarks(), null);
		col1.addChildNode(input);
		...
		// 流转意见
		input = InputGroup.getInGroup(true, "流转意见", "flowSuggestion",
				bookInfo.getFlowSuggestion(), null);
		col1.addChildNode(input);
		...
		bookFile.setBookId(id);
		String bookFileParam = DataConverter.convertObject2Json(bookFile);
		List<BookFile> bookFilelist = bookFileService
				.getFileByBookId(bookFileParam);
		if (bookFilelist.size() > 0) {
			for (BookFile i : bookFilelist) {
				String fileDown = "";
				fileDown += i.getFileName().toString() + "	"
						+ createDownButton(i.getId(), flag);
				// 创建第二行
				row = util.createRow(root);
				// 创建列
				col1 = util.createColumn(row, "12", "12", "12", "0");
