...
public class ChartCompletionPage2 {
	/**
	 * 初始化编印工作量完成情况数据页面
	 * @return 节点字符串
	 * */
	public String getListPage() {
		...
		util.createHeaderBar(root, "编印工作量完成情况");
		...
		InputGroup ig = (InputGroup) util.createRangeDatePickerGroup("资料汇交日期", "date1", "请输入资料汇交日期", "date2", "请输入资料汇交日期");
	...
	//	col2.addChildNode(searchButton);
		...
		Button button1 = Button.getButtonWithIcon("export1", css, "&nbsp;"+"导出海图制印工作统计表", i);
		column.addChildNode(button1);
	 ...
	 * @return 节点字符串
	 * */
	public String getProcessingListPage() {
		// 创建前端工具实例
		FrontUtil util = FrontUtil.getInstance();
		// 创建一个容器
		Base root = util.createRoot();
		util.createHeaderBar(root, "沿海港口航道图在编情况");
		util.createRowSpace(root);
		/** 创建按钮组行  开始*/
		// 创建行
		Base rowBg = util.createRow(root);
