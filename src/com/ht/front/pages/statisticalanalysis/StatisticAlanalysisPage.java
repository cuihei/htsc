 ...
 *
 */
public class StatisticAlanalysisPage {
	/**
	 * 初始化港口航道图完成情况数据页面
	 * @return 节点字符串
	 * */
	public String getListPage() {
		...
		util.createHeaderBar(root, "港口航道图完成情况");
		...
		InputGroup ig = (InputGroup) util.createRangeDatePickerGroup("审定完成日期", "date1", "请输入审定完成开始日期", "date2", "请输入审定完成结束日期");
		...
		/** 创建Grid行  开始*/
		// 创建Grid
		Base rowGrid = util.createGrid(root,"completion_status");
		rowSpace = util.createRowSpace(root);
		/** 创建Grid行  结束*//*
		rowBg = util.createRow(root);
		// 创建列
		Base col1 = util.createColumn(rowBg,"3","3", "3","0");
		// 创建列
		Base col2 = util.createColumn(rowBg,"3","3", "3","0");
		// 创建列
		Base col3 = util.createColumn(rowBg,"3","3", "3","0");
		InputGroup tb1 = InputGroup.getInGroup(true,"源数据完成数量月/累计", "source", null, "");
		col1.addChildNode(tb1);
		InputGroup tb2 = InputGroup.getInGroup(true,"纸海图完成数量月/累计", "paper", null, "");
		col2.addChildNode(tb2);
		InputGroup tb3 = InputGroup.getInGroup(true,"电子海图完成数量月/累计", "electronic", null, "");
		col3.addChildNode(tb3);		*/
		//返回节点字符串
		return root.getNode();
	}
}
