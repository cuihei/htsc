...
public class RuTaskPage {
	...
	public String getRuTaskPage(String processDefKey,List<User> users,boolean flag,boolean isRutask,boolean curRoleId){
		// 创建前端工具实例
       ...
       TextBox text = TextBox.getDefault("year", String.valueOf(year), "年份");
		...
		// 创建任务提交模态窗口
		Div modalDiv = buildTaskPerformModal(processDefKey,curRoleId);
		// 创建多任务提交模态窗口
	 ...
	 * @return
	 */
	private Div buildTaskPerformModal(String processDefKey,boolean curRoleId){
		...
		if (curRoleId) {
			Prop dialogDivProp = new Prop("style", "width:735px;margin:9% auto;");
			dialogDiv.addProp(dialogDivProp);
		} else {
			Prop dialogDivProp = new Prop("style", "width:500px;margin:9% auto;");
			dialogDiv.addProp(dialogDivProp);
		}
		
		CssClass css = new CssClass("form-search");
		Form form = Form.getInstance("importForm", css, null);
		// form表单添加属性
		...
		form.addChildNode(dialogDiv);
		modalDiv.addChildNode(form);
		...
		if (curRoleId) {
			Prop contentDivProp = new Prop("style", "width:250px;float:left");
			contentDiv.addProp(contentDivProp);
		} else {
			Prop contentDivProp = new Prop("style", "width:500px;float:left");
			contentDiv.addProp(contentDivProp);
		}
		...
		if(processDefKey.equals("CATALOG_MANAGEMENT")||processDefKey.equals("DATA_INPUT")||processDefKey.equals("DATA_BORROWING")||processDefKey.equals("DATA_RETURN")){
			...
			//Input subBtn=Input.getInput("button","submitOpinion", subCss, "提交","");
			Input subBtn=Input.getInput("button", "submitOpinion", subCss, "提交","");
			Prop prop = new Prop("onclick", "ru_task.submitOpinion()");
			subBtn.addProp(prop);
			headerDiv.addChildNode(subBtn);
		...
		//如果是 主任 质检 审定 科长角色则创建右侧窗口
		if(curRoleId) {
				
			/////////////////右侧 创建div 
			CssClass contentCssR = new CssClass("modal-content-r");			
			Div contentDivR = Div.getInstance(null, contentCssR, null);
			Prop contentPropR = new Prop("style","width:455px;float:left;background-color:#fff");
			contentDivR.addProp(contentPropR);
			dialogDiv.addChildNode(contentDivR);
			// 创建header div
			CssClass headerCssR = new CssClass("modal-header");
			Div headerDivR = Div.getInstance(null, headerCssR, null);
			contentDivR.addChildNode(headerDivR);
			// 构建关闭按钮
			CssClass closeCssR = new CssClass("close");
			Button closeBtnR = Button.getInstance(null, closeCssR, "&times;");
			Prop closePropR = new Prop();
			closePropR.setPropKey("data-dismiss");
			closePropR.setPropValue("modal");
			Prop closePropRs = new Prop();
			closePropRs.setPropKey("aria-hidden");
			closePropRs.setPropValue("true");
			// 绑定属性
			closeBtnR.addProp(closePropR);
			closeBtnR.addProp(closePropRs);
			headerDivR.addChildNode(closeBtnR);
			Prop textAreacss = new Prop("style","width:325px;height:72px;float:right;margin-left:10px; margin-top:0px;");
			/**
			 * 通告改正至开始
			 */
			// 创建标题div
			
			CssClass titleCssR1 = new CssClass("modal-title bk-fg-primary model-custom");
			Prop titleAreaprop1 = new Prop("style","width:100%;margin-top:10px; height: 30px; font-family: 微软雅黑; font-size: 24px;font-weight: bold margin-bottom: 10px;color: #34495E; clear:both");
			Div titleDivR1 = Div.getInstance("er_title1", titleCssR1, "错误说明一：");
			titleDivR1.addProp(titleAreaprop1);
			headerDivR.addChildNode(titleDivR1);
					
					
			/*CssClass backCssR = new CssClass("btn btn-default");
			Button backBtnR = Button.getInstance("file", backCssR, "上传图片");
			Prop propR = new Prop("onclick", "onclic(this)");
			backBtnR.addProp(propR);
			propR = new Prop("style", "width:20%; height:72px; float:left; background-color:#ececec");
			backBtnR.addProp(propR);
			headerDivR.addChildNode(backBtnR);*/
			//-------------------------------------------------------------------------
			// 上传错误图片
			Div div1 = Div.getInstance("div1", null, null);
			prop = new Prop("style", "width:80px; height:72px; float:left; background-color:#ececec");
			div1.addProp(prop);
			headerDivR.addChildNode(div1);
			//-------------------------------------------------------------------------

					
			
			//-------------------------------------------------------------------------
			Prop textAreaprop = new Prop("style","width:80%;margin-top:0px");
			InputGroup textArea1 = InputGroup.getTextAreaGroup("errtxt1","3", "");
			textAreaprop = new Prop("style","width:325px;height:72px;margin-top:0px;float:right;margin-left:10px; ");
			textArea1.addProp(textAreaprop);
			textArea1.addProp(textAreacss);
			headerDivR.addChildNode(textArea1);
			/////////////////////////////////////////////////////////////////////////////////////////
			

			
			CssClass titleCssR2 = new CssClass("modal-title bk-fg-primary model-custom");
			Prop titleAreaprop2 = new Prop("style","width:100%;margin-top:10px; height: 30px; font-family: 微软雅黑; font-size: 24px;font-weight: bold margin-bottom: 10px;color: #34495E; clear:both");
			Div titleDivR2 = Div.getInstance("er_title2", titleCssR2, "错误说明二：");
			titleDivR2.addProp(titleAreaprop2);
			headerDivR.addChildNode(titleDivR2);
					
			//-------------------------------------------------------------------------
			// 上传错误图片
			Div div2 = Div.getInstance("div2", null, null);
			prop = new Prop("style", "width:80px; height:72px; float:left; background-color:#ececec");
			div2.addProp(prop);
			headerDivR.addChildNode(div2);
			
			//-------------------------------------------------------------------------

			Prop textAreaprop2 = new Prop("style","width:80%;margin-top:0px");
			InputGroup textArea2= InputGroup.getTextAreaGroup("errtxt2","3", "");
			textAreaprop2 = new Prop("style","width:325px;height:72px;float:right;margin-left:10px; margin-top:0px;");

			textArea2.addProp(textAreaprop2);
			textArea2.addProp(textAreacss);
			headerDivR.addChildNode(textArea2);
			
			CssClass titleCssR3 = new CssClass("modal-title bk-fg-primary model-custom");
			Prop titleAreaprop3 = new Prop("style","width:100%;margin-top:10px; height: 30px; font-family: 微软雅黑; font-size: 24px;font-weight: bold margin-bottom: 10px;color: #34495E; clear:both");
			Div titleDivR3 = Div.getInstance("er_title3", titleCssR3, "错误说明三：");
			titleDivR3.addProp(titleAreaprop3);
			headerDivR.addChildNode(titleDivR3);
					
			//-------------------------------------------------------------------------
			// 上传错误图片
			Div div3 = Div.getInstance("div3", null, null);
			prop = new Prop("style", "width:80px; height:72px; float:left; background-color:#ececec");
			div3.addProp(prop);
			headerDivR.addChildNode(div3);
			//-------------------------------------------------------------------------

					
			Prop textAreaprop3 = new Prop("style","width:80%;margin-top:0px");
			InputGroup textArea3= InputGroup.getTextAreaGroup("errtxt3","3", "");
			textAreaprop3 = new Prop("style","width:325px;height:72px;float:right;margin-left:10px; margin-top:0px;");
			textArea3.addProp(textAreaprop3);
			textArea3.addProp(textAreacss);
			headerDivR.addChildNode(textArea3);
			
			////////////////////////// 备注
			
			CssClass titleCssR4 = new CssClass("modal-title bk-fg-primary model-custom");
			Prop titleAreaprop4 = new Prop("style","width:100%;margin-top:10px; height: 30px; font-family: 微软雅黑; font-size: 24px;font-weight: bold margin-bottom: 10px;color: #34495E; clear:both");
			Div titleDivR4 = Div.getInstance("er_title4", titleCssR4, "备注：");
			titleDivR4.addProp(titleAreaprop4);
			headerDivR.addChildNode(titleDivR4);
					
			//-------------------------------------------------------------------------
			// 
			Div div4 = Div.getInstance("div4", null, null);
			prop = new Prop("style", "width:100%; height:72px; float:left;");
			div4.addProp(prop);
			headerDivR.addChildNode(div4);
	...
	private Div buildMutiTaskPerformModal(){
		...
		Input subBtn=Input.getInput("button", "submitOpinion", subCss1, "提交","");
