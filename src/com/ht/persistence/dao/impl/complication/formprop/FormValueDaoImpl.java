...
public class FormValueDaoImpl extends BaseDaoImpl implements FormValueDao
{
	...
	public List<FormValue> getFormValueByProcessInstId(FormValue formValue)
	{
		...
		/*
		 * SQLQuery query1 = this.getSession().createSQLQuery(
		 * "select * from form_value t where t.PROCESS_INST_ID =?  order by t.modify_date,t.prop_key"
		 * ); query1.setString(0, formValue.getProcessInstId());
		 * query1.addEntity(FormValue.class); List<FormValue> list1 =
		 * query1.list(); List<Integer> lists=new ArrayList<Integer>(); for (int
		 * i = 0; i < list1.size(); i++) { if(list1.get(i).getFormBb()!=null) {
		 * int num =
		 * Integer.parseInt(list1.get(i).getFormBb().toString().replaceAll(
		 * "[^0-9]","")); lists.add(num); }
		 * 
		 * } String x="  "; if(lists.size()>0) { x="and t.FORM_BB  like '%"+
		 * Collections.max(lists)+"%'   "; }
		 */
		/////////////////////////////
		
		SQLQuery query = this.getSession().createSQLQuery(
		//		"select * from form_value t where t.PROCESS_INST_ID =? "+x+"  and t.form_id = ? order by t.modify_date,t.row_flag");
	...
	@Override
	public void addFormValueByTaskId(String processInstId, String loginUser,String taskinstid,String taskDefId){
