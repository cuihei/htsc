...
public class BooksDaoImpl extends BaseDaoImpl implements BooksDao {
	...
	public Books getBooksByChartNo(Books books) {
		...
		SQLQuery query = this.getSession().createSQLQuery("select * from books where chart_no='"+books.getChartNo()+"'  and status like '%审核通过%'  order by PUBLICATION_DATE desc");
		query.addEntity(Books.class);
		@SuppressWarnings("unchecked")
		List<Books> appList =  query.list();
		if(appList.size()>0){
