 ...
 *
 */
public class CorrectionNoticePersonAndDate extends PersonAndDateUtil{
	//编辑资料登记表 编辑员
	String registerCompilationPerson=null;
	//编辑资料登记表 编辑时间
	String registerCompilationDate=null;
	//编辑资料登记表 质检员
	String registerQualityPerson=null;
	//编辑资料登记表 质检时间
	String registerQualityDate=null;
	
	//最新改正项号记录表 编辑员
	String newCorrectionCompilationPerson=null;
	//最新改正项号记录表 编辑时间	
	String newCorrectionCompilationDate=null;
	//最新改正项号记录表 质检员
	String newCorrectionQualityPerson=null;
	//最新改正项号记录表 质检时间
	String newCorrectionQualityDate=null;
	...
	//部门质量检查记录表 检查意见
	String departmentQualityCompilationAdvice=null;
	//部门质量检查记录表 质检员
	...
	//中心质量检查记录表 检查意见
	String centreQualityvaladitonAdvice=null;
	//中心质量检查记录表 审定时间	
	...
	public CorrectionNoticePersonAndDate() {
		super();
		// TODO Auto-generated constructor stub
	}
	public CorrectionNoticePersonAndDate(List<Flows> flowList) {
		// 编辑资料登记表  编辑员、编辑时间
		Flows registerCompilationFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.REGISTERCOMPILATIONFLOW, flowList);
		if(null!=registerCompilationFlow){
			String userSignature = getUserSignature(registerCompilationFlow.getUserNo());
			if(userSignature!=null&&userSignature!=""){
				this.registerCompilationPerson=userSignature;
			}else{
				this.registerCompilationPerson=registerCompilationFlow.getUserName();
			}
			this.registerCompilationDate=formatDate(registerCompilationFlow.getEndTime());
		}
		//编辑资料登记表  质检员、质检时间	
		Flows registerQualityFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.REGISTERQUALITYFLOW, flowList);
		if(null!=registerQualityFlow){
			String userSignature = getUserSignature(registerQualityFlow.getUserNo());
			if(userSignature!=null&&userSignature!=""){
				this.registerQualityPerson=userSignature;
			}else{
				this.registerQualityPerson=registerQualityFlow.getUserName();
			}
			this.registerQualityDate=formatDate(registerQualityFlow.getEndTime());
		}
		// 最新改正项号记录表   编辑员、编辑时间
		Flows newCorrectionCompilationFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.NEWCORRECTIONCOMPILATIONFLOW, flowList);
		if(null!=newCorrectionCompilationFlow){
			String userSignature = getUserSignature(newCorrectionCompilationFlow.getUserNo());
			if(userSignature!=null&&userSignature!=""){
				this.newCorrectionCompilationPerson=userSignature;
			}else{
				this.newCorrectionCompilationPerson=newCorrectionCompilationFlow.getUserName();
			}
			this.newCorrectionCompilationDate=formatDate(newCorrectionCompilationFlow.getEndTime());
		}
		//最新改正项号记录表   质检员、质检时间	
		Flows newCorrectionQualityFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.NEWCORRECTIONQUALITYFLOW, flowList);
		if(null!=newCorrectionQualityFlow){
			String userSignature = getUserSignature(newCorrectionQualityFlow.getUserNo());
			if(userSignature!=null&&userSignature!=""){
				this.newCorrectionQualityPerson=userSignature;
			}else{
				this.newCorrectionQualityPerson=newCorrectionQualityFlow.getUserName();
			}
			this.newCorrectionQualityDate=formatDate(newCorrectionQualityFlow.getEndTime());
		}

		// 编辑情况记录表   编辑员、编辑时间
		Flows recordCompilationFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.RECORDCOMPILATIONFLOW, flowList);
		...
		// 部门质量检查记录表   编辑员、编辑时间
		Flows departmentQualityCompilationFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.DEPARTMENTQUALITYCOMPILATIONFLOW, flowList);
		...
		//部门质量检查记录表  质检员、质检时间	
		Flows departmentQualityQualityFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.DEPARTMENTQUALITYQUALITYFLOW, flowList);
		if(null!=departmentQualityQualityFlow){
			...
			this.departmentQualityCompilationAdvice = !departmentQualityQualityFlow.getTaskResult().equals("退回") ? "同意" : null;
		}
		//部门质量检查记录表   质检员、质检时间	
		Flows departmentQualityChiefFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.DEPARTMENTQUALITYCHIEFFLOW, flowList);
		...
		//中心质量检查记录表	质检员、质检时间
		Flows centreQualityQualityFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.CENTREQUALITYQUALITYFLOW, flowList);
		...
		//中心质量检查记录表 	审定员、审定时间
		Flows centreQualityvaladitonFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.CENTREQUALITYVALADITONFLOW, flowList);
		if(null!=centreQualityvaladitonFlow){
			...
			this.centreQualityvaladitonAdvice = !centreQualityvaladitonFlow.getTaskResult().equals("退回") ? "同意" : null;
		}
		//中心质量检查记录表   科长、操作时间	
		Flows centreQualityChiefFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.CENTREQUALITYCHIEFFLOW, flowList);
		...
		//总工程师
		Flows enginnerPersonFlow = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.ENGINNERPERSONFLOW, flowList);
	...
	public String getRegisterCompilationPerson() {
		return registerCompilationPerson;
	}
	public void setRegisterCompilationPerson(String registerCompilationPerson) {
		this.registerCompilationPerson = registerCompilationPerson;
	}
	public String getRegisterCompilationDate() {
		return registerCompilationDate;
	}
	public void setRegisterCompilationDate(String registerCompilationDate) {
		this.registerCompilationDate = registerCompilationDate;
	}
	public String getRegisterQualityPerson() {
		return registerQualityPerson;
	}
	public void setRegisterQualityPerson(String registerQualityPerson) {
		this.registerQualityPerson = registerQualityPerson;
	}
	public String getRegisterQualityDate() {
		return registerQualityDate;
	}
	public void setRegisterQualityDate(String registerQualityDate) {
		this.registerQualityDate = registerQualityDate;
	}
	public String getNewCorrectionCompilationPerson() {
		return newCorrectionCompilationPerson;
	}
	public void setNewCorrectionCompilationPerson(
			String newCorrectionCompilationPerson) {
		this.newCorrectionCompilationPerson = newCorrectionCompilationPerson;
	}
	public String getNewCorrectionCompilationDate() {
		return newCorrectionCompilationDate;
	}
	public void setNewCorrectionCompilationDate(String newCorrectionCompilationDate) {
		this.newCorrectionCompilationDate = newCorrectionCompilationDate;
	}
	public String getNewCorrectionQualityPerson() {
		return newCorrectionQualityPerson;
	}
	public void setNewCorrectionQualityPerson(String newCorrectionQualityPerson) {
		this.newCorrectionQualityPerson = newCorrectionQualityPerson;
	}
	public String getNewCorrectionQualityDate() {
		return newCorrectionQualityDate;
	}
	public void setNewCorrectionQualityDate(String newCorrectionQualityDate) {
		this.newCorrectionQualityDate = newCorrectionQualityDate;
	}
	...
	public String getDepartmentQualityCompilationAdvice() {
		return departmentQualityCompilationAdvice;
	}
	public void setDepartmentQualityCompilationAdvice(
			String departmentQualityCompilationAdvice) {
		this.departmentQualityCompilationAdvice = departmentQualityCompilationAdvice;
	}
	public String getCentreQualityvaladitonAdvice() {
		return centreQualityvaladitonAdvice;
	}
	public void setCentreQualityvaladitonAdvice(String centreQualityvaladitonAdvice) {
		this.centreQualityvaladitonAdvice = centreQualityvaladitonAdvice;
	}
	...
	public void setListMap(List<Flows> flowList) {
		...
		//创建
		Flows createFlows = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.CREATE, flowList);
		if(null!=createFlows){
		...
		//编辑 开始
		Flows compilationBeginFlows = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.COMPILATIONBEGIN, flowList);
		if(null!=compilationBeginFlows){
			...
			Flows compilationEndFlows = getPersonAndDateByTaskDefIds(CorrectionNoticeFlowConstants.COMPILATIONEND, flowList);
		...
		//部门质检
		List<Flows> departmenttestingFlowsList = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.DEPARTMENTTESTING, flowList);
		if(null!=departmenttestingFlowsList&&departmenttestingFlowsList.size()>0){
			...
			if(departmenttestingFlows!=null){
				...
				if (dates != null && departmenttestingFlows!=null&&departmenttestingFlows.getEndTime() != null)
				{
		...
		//质检后改图，质检改后确认
		//质检后改图
		List<Flows> testingChangeFlows = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.TESTINGCHANGE, flowList);
		//质检改后确认
		List<Flows> testingConfirmFlows = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.TESTINGCONFIRM, flowList);
				...
				if(testingChangeFlows.size()>j){
					...
					if(testingChange!=null&&testingConfirm!=null){
						...
						if(flag){
							...
							dates = null;
							if (testingConfirm!=null&&testingConfirm.getStartTime() != null)
							{
								dates = df.format(testingConfirm.getStartTime());
							}
							if (testingConfirm!=null&&dates != null &&  testingConfirm.getEndTime() != null)
							{
		...
		//中心审定
		List<Flows> centralValidationFlowsList = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.CENTRALVALIDATION, flowList);
		...
		//审定后改图
		List<Flows> valaditondChangeFlows = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.VALADITONDCHANGE, flowList);
		//审定改后确认
		List<Flows> valaditondConfirmFlows = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.VALADITONDCONFIRM, flowList);
		//获取 排除审定后改图需要的list
		List<Flows> excludeValaditondConfirmFlows = getFlowsListByTaskDefIds(CorrectionNoticeFlowConstants.EXCLUDEVALADITONDCONFIRM, flowList);
		//排除多余的审定后改图
