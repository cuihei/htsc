...
import com.ht.service.constant.experiencebook.constants.ElectronFlowConstants;
/**
 * 电子海图编绘 各节点信息
 * @author huodesheng
 *
 */
public class ElectronPersonAndDate extends PersonAndDateUtil{
	// 签字内容 编绘人
	...
	public ElectronPersonAndDate() {
		super();
		// TODO Auto-generated constructor stub
	}

	public ElectronPersonAndDate(List<Flows> flowList) {

		
		
		// 编绘作业计划，签字内容 编绘人，编绘时间
		Flows planCompilation = getPersonAndDateByTaskDefIds(ElectronFlowConstants.PLANCOMPILATION, flowList);
		...
		Flows planQuality = getPersonAndDateByTaskDefIds(ElectronFlowConstants.PLANQUALITY, flowList);
		if(null!=planQuality){
		...
		Flows productionAdviceFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.PRODUCTIONADVICEFLOW, flowList);
		...
		Flows chargeAdviceFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.CHARGEADVICEFLOW, flowList);
		...
		// 问题处理记录表  编绘人，编绘时间
		Flows problemCompilationFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.PROBLEMCOMPILATIONFLOW, flowList);
		...
		// 问题处理记录表  质检人，质检时间
		Flows problemQualityFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.PROBLEMQUALITYFLOW, flowList);
		...
		// 问题处理记录表 	通告改正至
		Flows problemNoticeFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.PROBLEMNOTICEFLOW, flowList);
		if(null!=problemNoticeFlow){
			this.problemNotice=problemNoticeFlow.getAdvice();
		}
		
		// 质检记录表  编绘人，编绘时间
		Flows testCompilationFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCOMPILATIONFLOW1, flowList);
		if(testCompilationFlow==null){
			testCompilationFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCOMPILATIONFLOW2, flowList);
		}
		...
		// 质检记录表 质检人，质检时间
		Flows testQualityFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTQUALITYFLOW, flowList);
		...
		// 质检记录表  	通告改正至
		Flows testNoticeFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTNOTICEFLOW1, ElectronFlowConstants.TESTNOTICEFLOW2, flowList);
		if(null!=testCompilationFlow){
		...
		// 质检结论表  编绘人，编绘时间
		Flows testConclusionCompilationFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCONCLUSIONCOMPILATIONFLOW1, flowList);
		if(testConclusionCompilationFlow==null){
			testConclusionCompilationFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCONCLUSIONCOMPILATIONFLOW2, flowList);
		}
		...
		// 质检结论表 质检人，质检时间
		Flows testConclusionQualityFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCONCLUSIONQUALITYFLOW, flowList);
		...
		Flows testConclusionApproveQualityFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCONCLUSIONAPPROVEQUALITYFLOW, flowList);
		...
		// 质检结论表	通告改正至
		Flows testConclusionNoticeFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.TESTCONCLUSIONNOTICEFLOW1, ElectronFlowConstants.TESTCONCLUSIONNOTICEFLOW2, flowList);
		...
		// 审定记录表  审定人，审定日期
		Flows valaditionFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITIONFLOW, flowList);
		...
		// 审定记录表 科长，科长审定时间
		Flows valaditonApproveFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONAPPROVEFLOW, flowList);
		...
		//审定记录表  通告改正至
		Flows valaditonNoticeFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONNOTICEFLOW1, ElectronFlowConstants.VALADITONNOTICEFLOW2, flowList);
		if(null!=valaditonNoticeFlow){
			this.valaditonNotice=valaditonNoticeFlow.getAdvice();
		}
		
		// 审定结论表 质检人，质检日期
		Flows valaditonConclusionQualityFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONCONCLUSIONQUALITYFLOW1, flowList);
		if(valaditonConclusionQualityFlow==null){
			valaditonConclusionQualityFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONCONCLUSIONQUALITYFLOW2, flowList);
		}
		...
		// 审定结论表 科长，科长审定时间
		Flows valaditonConclusionApproveFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONCONCLUSIONAPPROVEFLOW, flowList);
		...
		// 审定结论表 审定人，审定时间
		Flows valaditionConclusionFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITIONCONCLUSIONFLOW, flowList);
		...
		Flows valaditonDepartmentApproveFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONDEPARTMENTAPPROVEFLOW, flowList);
		...
		// 审定结论表 通告改正至
		Flows valaditonConclusionNoticeFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONCONCLUSIONNOTICEFLOW1, ElectronFlowConstants.VALADITONCONCLUSIONNOTICEFLOW2, flowList);
		if(null!=valaditonConclusionNoticeFlow){
			this.valaditonConclusionNotice=valaditonConclusionNoticeFlow.getAdvice();
		}
		
		//总工程师
		Flows enginnerPersonFlow = getPersonAndDateByTaskDefIds(ElectronFlowConstants.ENGINNERPERSONFLOW, flowList);
		...
		// 总工程师 通告改正至
		Flows enginnerNoticeFlow  = getPersonAndDateByTaskDefIds(ElectronFlowConstants.VALADITONCONCLUSIONNOTICEFLOW1, ElectronFlowConstants.VALADITONCONCLUSIONNOTICEFLOW2, flowList);
	...
	public void setListMap(List<Flows> flowList) {
		...
		List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
		EleFlows seaMapFlow = new EleFlows();
		//创建
		Flows createFlows = getPersonAndDateByTaskDefIds(ElectronFlowConstants.CREATE, flowList);
		if(null!=createFlows){
			seaMapFlow = new EleFlows();
		...
		//编   绘
		Flows compilationFlows = getPersonAndDateByTaskDefIds(ElectronFlowConstants.COMPILATION, flowList);
		...
		//部门质检
		Flows departmenttesting1Flows = getPersonAndDateByTaskDefIds(ElectronFlowConstants.DEPARTMENTTESTING1, flowList);
		if(null!=departmenttesting1Flows){
			...
			Flows departmenttesting2Flows = getPersonAndDateByTaskDefIds(ElectronFlowConstants.DEPARTMENTTESTING2, flowList);
		...
		//质检后改图，质检改后确认
		//质检后改图
		List<Flows> testingChangeFlows = getFlowsListByTaskDefIds(ElectronFlowConstants.TESTINGCHANGE, flowList);
		//质检改后确认
		List<Flows> testingConfirmFlows = getFlowsListByTaskDefIds(ElectronFlowConstants.TESTINGCONFIRM, flowList);
		...
		//中心审定
		Flows centralValidationFlows = getPersonAndDateByTaskDefIds(ElectronFlowConstants.CENTRALVALIDATION, flowList);
		...
		//审定后改图
		List<Flows> valaditondChangeFlows = getFlowsListByTaskDefIds(ElectronFlowConstants.VALADITONDCHANGE, flowList);
		//审定改后确认
		List<Flows> valaditondConfirmFlows = getFlowsListByTaskDefIds(ElectronFlowConstants.VALADITONDCONFIRM, flowList);
		//判空
