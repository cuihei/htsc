...
import com.ht.service.constant.experiencebook.constants.PaperSmallFlowConstants;
import com.ht.service.constant.experiencebook.constants.PaperSmallFlowConstants;

public class PaperSmallPersonAndDate extends PersonAndDateUtil{
	//编绘记录，编绘员
    ...
    //中心质量验收记录，总工程师
    String coreQualityRecordPerson=null;
    
    //中心质量验收记录，日期
    String coreQualityRecordDate=null;
    
	public PaperSmallPersonAndDate() {
		super();
	...
	public PaperSmallPersonAndDate(List<Flows> flowList) {
		
		//编绘记录， 编绘人，编绘时间
		Flows compilationFlow = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.COMPILATIONFLOW, flowList);
		...
		//质量检查记录	质检员、质检日期
		Flows qualityFlow = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.QUALITYFLOW, flowList);
		if(null!=qualityFlow){
		...
		//制图事业部结论	制图事业部主任、日期
		Flows departmentFlow = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.DEPARTMENTFLOW, flowList);
		...
		//中心质量审定记录	审定员、审定日期
		Flows coreValidationFlow = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.COREVALIDATIONFLOW, flowList);
		...
		Flows departmentQualityFlow = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.DEPARTMENTQUALITYFLOW, flowList);
		...
		//总工程师质量验收记录表	总工程师、验收日期
		Flows coreQualityRecordFlow = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.COREQUALITYRECORDFLOW, flowList);
		if(null!=coreQualityRecordFlow){
			String userSignature = getUserSignature(coreQualityRecordFlow.getUserNo());
			if(userSignature!=null&&userSignature!=""){
				this.coreQualityRecordPerson=userSignature;
			}else{
				this.coreQualityRecordPerson=coreQualityRecordFlow.getUserName();
			}
			this.coreQualityRecordDate=formatDate(coreQualityRecordFlow.getEndTime());
		}
	...
	public String getCoreQualityRecordPerson() {
		return coreQualityRecordPerson;
	}

	public void setCoreQualityRecordPerson(String coreQualityRecordPerson) {
		this.coreQualityRecordPerson = coreQualityRecordPerson;
	}

	public String getCoreQualityRecordDate() {
		return coreQualityRecordDate;
	}

	public void setCoreQualityRecordDate(String coreQualityRecordDate) {
		this.coreQualityRecordDate = coreQualityRecordDate;
	}
	...
	public void setListMap(List<Flows> flowList) {
		...
		//创建
		/*Flows createFlows = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.CREATE, flowList);
		if(null!=createFlows){
		...
		//编   绘
		Flows compilationFlows = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.COMPILATIONFLOW, flowList);
		...
		//部门质检
		Flows departmenttesting1Flows = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.QUALITYFLOW, flowList);
		...
		//质检后改图，质检改后确认
		//质检后改图
		/*List<Flows> testingChangeFlows = getFlowsListByTaskDefIds(PaperSmallFlowConstants.TESTINGCHANGE, flowList);
		//质检改后确认
		List<Flows> testingConfirmFlows = getFlowsListByTaskDefIds(PaperSmallFlowConstants.TESTINGCONFIRM, flowList);
		...
		//中心审定
		Flows centralValidationFlows = getPersonAndDateByTaskDefIds(PaperSmallFlowConstants.COREVALIDATIONFLOW, flowList);
		...
		/*//审定后改图
		List<Flows> valaditondChangeFlows = getFlowsListByTaskDefIds(PaperSmallFlowConstants.VALADITONDCHANGE, flowList);
		//审定改后确认
		List<Flows> valaditondConfirmFlows = getFlowsListByTaskDefIds(PaperSmallFlowConstants.VALADITONDCONFIRM, flowList);
		//判空
