	...
	public Map<String, Object> PaperQualityInspectMap(String processInstId,String type)
		...
		try {
			...
			if(type=="PAPER"){
				...
				//过程检查意见,作业部门负责人
				qualityTestReport.setOPERATION_PEOPLE(pd.getTestConclusionApprovePerson());
				//过程检查意见,作业部门负责人时间
				...
				//最终检查意见,作业部门负责人
				qualityTestReport.setFINAL_PEOPLE(pd.getValaditonApprovePerson());
				//最终检查意见,作业部门负责人时间
				...
				qualityTestReport.setOPERATION_PEOPLE(pd.getTestConclusionApprovePerson());
				//过程检查意见,作业部门负责人时间
				qualityTestReport.setOPERATION_DATE(pd.getTestConclusionApproveDate());
				//最终检查意见
				qualityTestReport.setFINAL_OPINION(pd.getValaditonDepartmentApproveAdvice());
				//最终检查意见,作业部门负责人
				qualityTestReport.setFINAL_PEOPLE(pd.getValaditonApprovePerson());
				//最终检查意见,作业部门负责人时间
				qualityTestReport.setFINAL_DATE(pd.getValaditonApproveDate());
				//验收结论
				qualityTestReport.setCONCLUSION_OPINION(pd.getEnginnerAdvice());
				//验收结论 作业部门负责人
				qualityTestReport.setCONCLUSION_PEOPLE(pd.getEnginnerPerson());
	...
	public Map<String, Object> PaperSelfQualityEvaluateMap(
		...
		try {
			...
			if(type=="PAPER"){
				qualityEvaluate.setTYPE("纸海图");
				PaperPersonAndDate pd = new PaperPersonAndDate(flowListAll);
				qualityEvaluate.setDATE(pd.getTestConclusionQualityDate() == null ? "" : pd.getTestConclusionQualityDate());
				/**
				 * 纸海图质检记录表
				 */
				map= base.getPaperValidationDefective(workProcessInstId, pd.getValaditionPerson(), pd.getValaditionDate(), pd.getValaditonApprovePerson(),pd.getValaditonApproveDate(), null);
				//List<DefectForm> defectFormList = defectFormDao.getDefectFormListByProcessInstId(workProcessInstId, ChartStatusConstants.APPROVE_LIKE);  //审定记录 2018.9.27
				List<DefectForm> defectFormList = defectFormDao.getDefectFormListByProcessInstId(workProcessInstId, ChartStatusConstants.QUALITY_LIKE);  //质检记录 2018.9.27
				String total = null;
				String coefficient = null;
				String actual = null;
				String grading = null;
				...
				qualityEvaluate.setMAP_NAME(complicationTask.getTaskName());
				/**
				 * 审定记录结论表  这是错的 应该是质检记录表 2019、9.27
				 */
				//PaperValidationConclusionRecord vc = new PaperValidationConclusionRecord(); //这是审定
				//List<List<FormValue>> vcList = base.getFormValueByRowFlag(vc.VALIDATION_CONCLUSION_FROM_ID, workProcessInstId);//这是审定
        		PaperQualityInspectionConclusionRecord  vc = new PaperQualityInspectionConclusionRecord(); //这是质检
				List<List<FormValue>> vcList = base.getFormValueByRowFlag(vc.QUALITY_INSPECTION_FORM_ID, workProcessInstId); //这是质检
				// 质检结论
				String sdjl = null;
				// 质检评分及等级
				String zlpf="";
				...
				if(vcList!=null){
					if(vcList.size()>0){
						for (int i = 0; i < vcList.size(); i++)
						{
							vc = new PaperQualityInspectionConclusionRecord();
							List<FormValue> vcListI = vcList.get(i);
							vc.setDataProps(vcListI);
							Map<String, Object> mapI = vc.getPaperQualityInspectionConclusionRecordMap();
							sdjl = mapI.get("text_zhtzjjlb_jl") == null ? null : mapI.get("text_zhtzjjlb_jl").toString();
							zlpf = mapI.get("text_zhtzjjlb_zlpf") == null ? null : mapI.get("text_zhtzjjlb_zlpf").toString();
							zldj = mapI.get("text_zhtzjjlb_zldj") == null ? "" : mapI.get("text_zhtzjjlb_zldj").toString();
						}
						//最终得分
						qualityEvaluate.setZZDF(zlpf);
				...
				/**
				 * 电子海图质检结论记录表
				 * EleQualityInspectionConclusionRecord
				 */
				EleQualityInspectionConclusionRecord vc = new EleQualityInspectionConclusionRecord();
				List<List<FormValue>> vcList = base.getFormValueByRowFlag(vc.QUALITY_INSPECTION_FORM_ID, workProcessInstId);
				// 审定结论
				String sdjl = "";
				// 质量评分及等级
				String pfdj = "";
				...
				if(vcList!=null){
					if(vcList.size()>0){
						for (int i = 0; i < vcList.size(); i++)
						{
							vc = new EleQualityInspectionConclusionRecord();
							List<FormValue> vcListI = vcList.get(i);
							vc.setDataProps(vcListI);
							Map<String, Object> mapI = vc.getQualityInspectionConclusionRecordMap();
							sdjl = mapI.get("text_dzhtzjjlb_jl") == null ? "" : mapI.get("text_dzhtzjjlb_jl").toString();
							zldj = mapI.get("text_dzhtzjjlb_zldj") == null ? "" : mapI.get("text_dzhtzjjlb_zldj").toString();
							zlpf = mapI.get("text_dzhtzjjlb_zlpf") == null ? "" : mapI.get("text_dzhtzjjlb_zlpf").toString();
